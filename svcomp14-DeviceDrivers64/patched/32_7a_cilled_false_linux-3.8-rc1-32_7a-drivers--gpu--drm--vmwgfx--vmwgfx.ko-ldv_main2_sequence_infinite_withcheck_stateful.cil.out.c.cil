...
{ struct vmw_sw_context *sw_context ;
  ...
  ret = vmw_execbuf_fence_commands(file_priv, dev_priv, & fence, tmp___20);
  if (ret != 0) {
...
{ uint32_t num_displays ;
  ...
  if (((struct vmw_display_unit *)__mptr___0 + 0xfffffffffffffc58UL)->unit < num_displays && (int )du->pref_active) {
    tmp___0 = 1;
  } else {
    tmp___0 = 2;
  }
...
void ldv_mutex_lock_643(struct mutex *ldv_func_arg1 ) ;
 static bool ttm_bo_is_reserved(struct ttm_buffer_object *bo ) 
{ int tmp ;
...
{ uint64_t tmp ;
 ...
 static struct svga3d_surface_desc  const  *svga3dsurface_get_desc(SVGA3dSurfaceFormat format ) 
{ 
 ...
 static void svga3dsurface_get_size_in_blocks(struct svga3d_surface_desc  const  *desc ,
...
{ 
 ...
 static bool svga3dsurface_is_planar_surface(struct svga3d_surface_desc  const  *desc ) 
{ 
 ...
 static u32 svga3dsurface_calculate_pitch(struct svga3d_surface_desc  const  *desc ,
...
{ u32 pitch ;
 ...
 static u32 svga3dsurface_get_image_buffer_size(struct svga3d_surface_desc  const  *desc ,
                                                        struct drm_vmw_size  const  *size ,
                                                        u32 pitch ) 
...
{ struct vmw_private *dev_priv ;
  ...
  if (res->id != -1) {
    ...
    dev_priv->used_memory_size -= (uint32_t )res->backup_size;
