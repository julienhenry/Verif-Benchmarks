...
{ struct snd_rawmidi *rawmidi ;
 ...
 static unsigned short snd_rawmidi_file_flags(struct file *file ) 
{ 
 ...
 static int snd_rawmidi_ready(struct snd_rawmidi_substream *substream ) 
{ struct snd_rawmidi_runtime *runtime ;
 ...
 static int snd_rawmidi_ready_append(struct snd_rawmidi_substream *substream ,
                                             size_t count ) 
{ struct snd_rawmidi_runtime *runtime ;
...
{ struct snd_rawmidi_runtime *runtime ;
 ...
 static void snd_rawmidi_output_trigger(struct snd_rawmidi_substream *substream ,
                                                int up___0 ) 
{ 
...
{ unsigned long flags ;
  ...
  if (count == 1) {
    ...
    runtime->hw_ptr %= runtime->buffer_size;
    runtime->avail += (size_t )count1;
    count -= count1;
    result += count1;
    if (count > 0) {
      buffer += (unsigned long )count1;
      count1 = count;
      if ((int )((unsigned int )runtime->buffer_size - (unsigned int )runtime->avail) < count1) {
        count1 = (int )((unsigned int )runtime->buffer_size - (unsigned int )runtime->avail);
        runtime->xruns += (size_t )(count - count1);
      }
      if (count1 > 0) {
...
{ unsigned long flags ;
  ...
  runtime->appl_ptr += (unsigned long )count1;
  runtime->appl_ptr %= runtime->buffer_size;
  runtime->avail -= (unsigned long )count1;
  spin_unlock_irqrestore(& runtime->lock, flags);
  result += count1;
  count -= count1;
  ldv_24730: ;
...
{ unsigned long flags ;
  ...
  ldv__builtin_expect(__ret_warn_on != 0, 0L);
  runtime->hw_ptr += (size_t )count;
  runtime->hw_ptr %= runtime->buffer_size;
  runtime->avail += (size_t )count;
  substream->bytes += (size_t )count;
  if (count > 0) {
...
{ struct snd_rawmidi *rmidi ;
  ...
  if ((int )rmidi->info_flags & 1) {
    ...
    if ((unsigned int )*((unsigned char *)substream + 24UL) != 0U) {
      ...
      runtime = substream->runtime;
      if ((unsigned int )*((unsigned char *)runtime + 8UL) != 0U) {
        tmp___1 = (char *)"OSS compatible";
      } else {
        tmp___1 = (char *)"native";
      }
      snd_iprintf(buffer, "  Mode         : %s\n  Buffer size  : %lu\n  Avail        : %lu\n",
                  tmp___1, runtime->buffer_size, runtime->avail);
    }
    __mptr___0 = (struct list_head  const  *)substream->list.next;
    substream = (struct snd_rawmidi_substream *)__mptr___0;
    ldv_26226: ;
...
{ struct file *var_group1 ;
  ...
  if (ldv_s_snd_rawmidi_f_ops_file_operations == 0) {
    ...
    ldv_s_snd_rawmidi_f_ops_file_operations ++;
  }
  goto ldv_26500;
  case 1: ;
