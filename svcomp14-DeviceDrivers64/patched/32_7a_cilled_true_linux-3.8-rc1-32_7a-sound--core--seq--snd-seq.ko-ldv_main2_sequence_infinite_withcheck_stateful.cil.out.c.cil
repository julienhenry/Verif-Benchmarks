...
{ void *tmp ;
 ...
 static struct device *snd_card_get_device_link(struct snd_card *card ) 
{ struct device *tmp ;
...
extern int snd_register_device_for_dev(int  , struct snd_card * , int  , struct file_operations  const  * ,
 ...
 static int snd_register_device(int type , struct snd_card *card , int dev ,
...
int snd_seq_event_dup(struct snd_seq_pool *pool , struct snd_seq_event *event , struct snd_seq_event_cell **cellp ,
 ...
 static int snd_seq_unused_cells(struct snd_seq_pool *pool ) 
{ int tmp ;
  int tmp___0 ;
 ...
 static int snd_seq_total_cells(struct snd_seq_pool *pool ) 
{ int tmp ;

  {
  if ((unsigned long )pool != (unsigned long )((struct snd_seq_pool *)0)) {
    tmp = pool->total_elements;
  } else {
    tmp = 0;
  }
  return (tmp);
...
static int snd_seq_deliver_single_event(struct snd_seq_client *client , struct snd_seq_event *event ,
 ...
 static mm_segment_t snd_enter_user(void) 
{ mm_segment_t fs ;
 ...
 static void snd_leave_user(mm_segment_t fs ) 
{ struct thread_info *tmp ;
 ...
 static unsigned short snd_seq_file_flags(struct file *file ) 
{ 
 ...
 static int snd_seq_write_pool_allocated(struct snd_seq_client *client ) 
{ int tmp ;
...
{ struct snd_seq_client *client ;
  ...
  if (((int )cell->event.flags & 12) == 4) {
    tmpev = cell->event;
    tmpev.data.ext.len &= 1073741823U;
...
{ struct snd_seq_client *client ;
  ...
  if ((unsigned int )cell->event.type == 5U) {
    ...
    goto ldv_25402;
    case 1: 
    ev->time.time.tv_nsec += (ev->data.note.duration % 1000U) * 1000000U;
    ev->time.time.tv_sec += ev->data.note.duration / 1000U + ev->time.time.tv_nsec / 1000000000U;
    ev->time.time.tv_nsec %= 1000000000U;
    goto ldv_25402;
    }
    ldv_25402: 
...
{ struct snd_seq_client_port *port ;
  ...
  if ((int )info.flags & 1) {
    tmp___1 = (int )info.addr.port;
  } else {
    tmp___1 = -1;
  }
  port = snd_seq_create_port(client, tmp___1);
...
{ struct list_head *p ;
  ...
  if (is_src != 0) {
    tmp___1 = (int )s->info.dest.port;
  } else {
    tmp___1 = (int )s->info.sender.port;
  }
  if (is_src != 0) {
    tmp___2 = (int )s->info.dest.client;
  } else {
    tmp___2 = (int )s->info.sender.client;
  }
  snd_iprintf(buffer, "%d:%d", tmp___2, tmp___1);
  if ((s->info.flags & 2U) != 0U) {
    if ((s->info.flags & 4U) != 0U) {
      tmp___3 = 114;
    } else {
      tmp___3 = 116;
    }
    snd_iprintf(buffer, "[%c:%d]", tmp___3, (int )s->info.queue);
  }
...
{ struct snd_seq_client_port *p ;
  ...
  goto ldv_25995;
  ldv_25994: 
  if ((p->capability & 16U) != 0U) {
    tmp = 88;
  } else {
    tmp = 45;
  }
  if ((p->capability & 128U) != 0U) {
    tmp___0 = 45;
  } else {
    tmp___0 = 101;
  }
  if ((p->capability & 2U) != 0U) {
    if ((p->capability & 64U) != 0U) {
      tmp___1 = 87;
    } else {
      tmp___1 = 119;
    }
  ...
  if ((int )p->capability & 1) {
    if ((p->capability & 32U) != 0U) {
      tmp___3 = 82;
    } else {
      tmp___3 = 114;
    }
...
{ struct file *var_group1 ;
  ...
  if (ldv_s_snd_seq_f_ops_file_operations == 0) {
    ...
    ldv_s_snd_seq_f_ops_file_operations ++;
  }
  goto ldv_26048;
  case 1: ;
...
{ 
 ...
 static int snd_seq_pool_available(struct snd_seq_pool *pool ) 
{ int tmp ;
 ...
 static int snd_seq_output_ok(struct snd_seq_pool *pool ) 
{ int tmp ;
...
{ int len ;
  ...
  if (in_kernel != 0) {
    tmp___0 = (int (*)(void * , void * , int  ))(& seq_copy_in_kernel);
  } else {
    tmp___0 = (int (*)(void * , void * , int  ))(& seq_copy_in_user);
  }
  err = snd_seq_dump_var_event(event, tmp___0, (void *)(& buf));
  if (err < 0) {
    tmp___1 = err;
  } else {
    tmp___1 = newlen;
...
void snd_seq_timer_delete(struct snd_seq_timer **tmr ) ;
 static int snd_seq_compare_tick_time(snd_seq_tick_time_t *a , snd_seq_tick_time_t *b ) 
{ 
 ...
 static int snd_seq_compare_real_time(snd_seq_real_time_t *a , snd_seq_real_time_t *b ) 
{ 
 ...
 static void snd_seq_sanity_real_time(snd_seq_real_time_t *tm ) 
{ 

  {
  goto ldv_23333;
  ldv_23332: 
  tm->tv_nsec -= 1000000000U;
  (tm->tv_sec) ++;
  ldv_23333: ;
 ...
 static void snd_seq_inc_real_time(snd_seq_real_time_t *tm , snd_seq_real_time_t *inc ) 
{ 
...
{ int dest ;
 ...
 static int check_access(struct snd_seq_queue *q , int client ) 
{ 
...
{ unsigned long flags ;
 ...
 static void queue_access_unlock(struct snd_seq_queue *q ) 
{ unsigned long flags ;
  raw_spinlock_t *tmp ;
...
{ struct snd_seq_prioq *f ;
 ...
 static int compare_timestamp(struct snd_seq_event *a , struct snd_seq_event *b ) 
{ int tmp ;
  int tmp___0 ;
 ...
 static int compare_timestamp_rel(struct snd_seq_event *a , struct snd_seq_event *b ) 
{ 
...
{ 
 ...
 static int prioq_match(struct snd_seq_event_cell *cell , int client , int timestamp ) 
{ 
...
extern int snd_timer_pause(struct snd_timer_instance * ) ;
 static void snd_seq_timer_update_tick(struct snd_seq_timer_tick *tick , unsigned long resolution ) 
{ 

  {
  if (tick->resolution != 0UL) {
    tick->fraction += resolution;
    tick->cur_tick += (snd_seq_tick_time_t )(tick->fraction / tick->resolution);
    tick->fraction %= tick->resolution;
  }
  return;
}
}
 static void snd_seq_sanity_real_time___0(snd_seq_real_time_t *tm ) 
{ 
 ...
 static void snd_seq_inc_time_nsec(snd_seq_real_time_t *tm , unsigned long nsec ) 
{ 

  {
  tm->tv_nsec += (unsigned int )nsec;
...
{ unsigned int s ;
  ...
  if (tmr->tempo <= 999999U) {
    ...
    tmr->tick.resolution += (unsigned long )s;
  }
...
{ snd_seq_real_time_t cur_time ;
  ...
  if ((unsigned int )*((unsigned char *)tmr + 0UL) != 0U) {
    ...
    if (usec < 0) {
      ...
      cur_time.tv_nsec += (unsigned int )(usec * 1000);
      cur_time.tv_sec += (unsigned int )tm.tv_sec - (unsigned int )tmr->last_update.tv_sec;
    }
...
{ int err ;
 ...
 static int addr_match(struct snd_seq_addr *r , struct snd_seq_addr *s ) 
{ 
...
{ struct snd_seq_port_info portinfo ;
  ...
  if ((unsigned long )portname != (unsigned long )((char *)0)) {
    tmp = (char const   *)portname;
  } else {
    tmp = "Unamed port";
  }
  strlcpy((char *)(& portinfo.name), tmp, 64UL);
