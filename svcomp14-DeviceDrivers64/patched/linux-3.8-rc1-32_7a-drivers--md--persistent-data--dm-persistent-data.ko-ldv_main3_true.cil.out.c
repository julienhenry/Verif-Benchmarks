...
{ unsigned char *entry ;
  ...
  entry += (unsigned long )(info->value_type.size * index);
  return ((void *)entry);
}
}
...
{ unsigned int tmp ;
  ...
  {
  if (nr_entries_in_last_block != 0U) {
    tmp = 1U;
  } else {
    tmp = 0U;
  }
  return (tmp + nr_full_blocks);
}
}
...
{ int r ;
  ...
  int (*tmp___1)(struct resize *resize ) ;
    ...
    if (tmp___5 != 0L) {
      ...
      if (tmp___4 != 0) {
        ...
        printk("\vdevice-mapper: block manager: validator mismatch (old=%s vs new=%s) for block %llu\n",
               (aux->validator)->name, tmp___7, (unsigned long long )tmp___3);
      }
      return (-22);
    }
  }
...
{ struct buffer_aux *aux ;
  ...
  tmp___2 = ldv__builtin_expect((unsigned long )p == (unsigned long )((void *)0),
                                0L);
  if (tmp___2 != 0L) {
    return (-11);
  }
...
{ int tmp ;
 ...
 static void __set_bit_le(int nr , void *addr ) 
{ 
 ...
 static void __clear_bit_le(int nr , void *addr ) 
{ 
...
{ int r ;
  ...
  _r /= (sector_t )ll->entries_per_block;
  ...
  _r___0 /= (sector_t )ll->entries_per_block;
  ...
  _r___1 /= (sector_t )ll->entries_per_block;
...
{ int r ;
  ...
  if (index_end - 1ULL != i) {
    bit_end = ll->entries_per_block;
  } else {
    bit_end = (uint32_t )end;
  }
  ...
  r = sm_find_free(tmp, tmp___0, bit_end, & position);
  if (r == -28) {
...
{ int r ;
  ...
  if (old != 0U && ref_count == 0U) {
    *ev = 2;
    ...
    _min2 = bit;
    if (_min1 < _min2) {
      ie_disk.none_free_before = _min1;
    } else {
      ie_disk.none_free_before = _min2;
    }
  }
...
{ 
 ...
 static int dm_sm_get_count(struct dm_space_map *sm , dm_block_t b , uint32_t *result ) 
{ int tmp ;
 ...
 static int dm_sm_count_is_more_than_one(struct dm_space_map *sm , dm_block_t b ,
                                                 int *result ) 
{ int tmp ;
...
{ int tmp ;
 ...
 static unsigned int dm_hash_block(dm_block_t b , unsigned int hash_mask ) 
...
{ int r ;
  ...
  __ret = __builtin_memcpy((void *)(& rn->keys), (void const   *)(& ln->keys) + (unsigned long )nr_left,
                           __len);
  if ((int )ln->header.flags & 1) {
    size = 8UL;
  } else {
    size = (unsigned long )(s->info)->value_type.size;
  }
...
{ int r ;
  ...
  goto ldv_29289;
  ldv_29288: 
  if ((unsigned int )level != info->levels - 1U) {
    tmp = & root;
  } else {
    tmp = 0;
  }
  r = find_highest_key(& spine, root, result_keys + (unsigned long )level, tmp);
  if (r == -61) {
    r = 0;
    goto ldv_29287;
  } else
...
{ uint32_t nr_left ;
  ...
  if (shift___0 < 0) {
    ...
    tmp___9 = ldv__builtin_expect((__le32 )shift___0 > right->header.max_entries,
                                  0L);
    if (tmp___9 != 0L) {
      __asm__  volatile   ("1:\tud2\n.pushsection __bug_table,\"a\"\n2:\t.long 1b - 2b, %c0 - 2b\n\t.word %c1, 0\n\t.org 2b+%c2\n.popsection": : "i" ((char *)"/work/ldvuser/exper_fp/work/current--X--drivers--X--defaultlinux-3.10-rc1.tar--X--32_7a--X--cpachecker/linux-3.10-rc1.tar/csd_deg_dscv/574/dscv_tempdir/dscv/ri/32_7a/drivers/md/persistent-data/dm-btree-remove.c.prepared"),
                           "i" (122), "i" (12UL));
...
{ unsigned int level ;
  ...
  void *tmp___3 ;
  int tmp___4 ;
  struct dm_btree_value_type *tmp___5 ;
  ...
  ldv_28783: 
  if (level == last_level) {
    tmp___5 = & info->value_type;
  } else {
    tmp___5 = & le64_type;
  }
  r = remove_raw(& spine, info, tmp___5, root, *(keys + (unsigned long )level), (unsigned int *)(& index));
  if (r < 0) {
    goto ldv_28780;
  }
  tmp = shadow_current(& spine);
