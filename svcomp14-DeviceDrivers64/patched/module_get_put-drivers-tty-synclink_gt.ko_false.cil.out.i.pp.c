...
{ int i ;
 ...
 static int sanity_check(struct slgt_info *info , char *devname , char const   *name ) 
{ 
...
{ struct slgt_info *info ;
 ...
 static void line_info(struct seq_file *m , struct slgt_info *info ) 
{ char stat_buf[30U] ;
  unsigned long flags ;
  raw_spinlock_t *tmp ;
...
{ struct tty_struct *tty ;
  ...
  ldv_36841: ;
  if (i < count) {
    info->rbuf_index += (unsigned int )i;
...
{ struct slgt_info *info ;
  ...
  info = (struct slgt_info *)__mptr + 1152921504606845504UL;
  if ((unsigned long )info == (unsigned long )((struct slgt_info *)0)) {
    return;
  }
...
{ unsigned long flags ;
  ...
  info = (struct slgt_info *)__mptr + 1152921504606846968UL;
  tmp = spinlock_check(& info->lock);
  flags = _raw_spin_lock_irqsave(tmp);
  get_signals(info);
  spin_unlock_irqrestore(& info->lock, flags);
...
static struct slgt_info *alloc_dev(int adapter_num ,
                                   int port_num , struct pci_dev *pdev ) 
{ struct slgt_info *info ;
  void *tmp ;
  struct lock_class_key __key ;
  ...
  if (tmp___0 == 30) {
    ...
    if (0) {
      ...
      if (ldv_s_hdlcdev_ops_net_device_ops == 0) {
        ...
        ldv_s_hdlcdev_ops_net_device_ops ++;
      }
      goto ldv_37978;
      case_3: ;
