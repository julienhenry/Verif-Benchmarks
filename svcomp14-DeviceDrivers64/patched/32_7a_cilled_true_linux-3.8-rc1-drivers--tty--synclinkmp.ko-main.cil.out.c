...
extern void tty_port_destroy(struct tty_port * ) ;
 static bool tty_port_cts_enabled(struct tty_port *port ) 
{ 
...
{ 
 ...
 static int sanity_check(SLMP_INFO *info , char *name , char const   *routine ) 
{ 
...
{ SLMP_INFO *info ;
 ...
 static void line_info(struct seq_file *m , SLMP_INFO *info ) 
{ char stat_buf[30U] ;
  unsigned long flags ;
  raw_spinlock_t *tmp ;
...
{ unsigned int cflag ;
  ...
  info->timeout += 5;
  if ((int )cflag < 0) {
    info->port.flags |= 67108864UL;
  } else {
    info->port.flags &= 4227858431UL;
  }
  if ((cflag & 2048U) != 0U) {
    info->port.flags &= 4261412863UL;
  } else {
    info->port.flags |= 33554432UL;
  }
...
{ int i ;
  ...
  (info->port_array[0])->last_mem_alloc += 1024U;
...
{ struct resource *tmp ;
  ...
  info->lcr_base += (unsigned long )info->lcr_offset;
  ...
  info->sca_base += (unsigned long )info->sca_offset;
  ...
  info->statctrl_base += (unsigned long )info->statctrl_offset;
...
{ SLMP_INFO *info ;
  ...
  if ((unsigned long )info == (unsigned long )((SLMP_INFO *)0)) {
    ...
    info->phys_lcr_base &= 4294963200U;
    info->sca_offset = info->phys_sca_base & 4095U;
    info->phys_sca_base &= 4294963200U;
    info->statctrl_offset = info->phys_statctrl_base & 4095U;
    info->phys_statctrl_base &= 4294963200U;
    info->bus_type = 5U;
  ...
  if (framesize != 0U) {
    if (info->max_frame_size < framesize) {
      ...
      __ret = __builtin_memcpy((void *)ptmp, (void const   *)info->rx_buf_list_ex[index].virt_addr,
                               __len);
      ptmp += (unsigned long )partial_count;
...
{ unsigned short interval ;
  ...
  read_status_reg(info);
  dest += (unsigned long )sca_pci_load_interval;
  src += (unsigned long )sca_pci_load_interval;
