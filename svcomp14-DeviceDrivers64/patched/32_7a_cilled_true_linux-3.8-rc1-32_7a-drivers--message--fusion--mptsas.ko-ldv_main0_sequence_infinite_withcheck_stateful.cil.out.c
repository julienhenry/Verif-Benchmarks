...
static void mptsas_parse_device_info(struct sas_identify *identify , struct mptsas_devinfo *device_info ) ;
 static void mptsas_set_rphy(MPT_ADAPTER *ioc , struct mptsas_phyinfo *phy_info ,
                                     struct sas_rphy *rphy ) ;
...
{ struct fw_event_work *fw_event ;
 ...
 static MPT_ADAPTER *phy_to_ioc(struct sas_phy *phy ) 
{ struct Scsi_Host *shost ;
 ...
 static MPT_ADAPTER *rphy_to_ioc(struct sas_rphy *rphy ) 
{ struct Scsi_Host *shost ;
  struct Scsi_Host *tmp ;
...
{ struct mptsas_portinfo *port_info ;
 ...
 static struct sas_rphy *mptsas_get_rphy(struct mptsas_phyinfo *phy_info ) 
{ 
...
{ 
 ...
 static struct sas_port *mptsas_get_port(struct mptsas_phyinfo *phy_info ) 
{ 
 ...
 static void mptsas_set_port(MPT_ADAPTER *ioc , struct mptsas_phyinfo *phy_info ,
                                     struct sas_port *port ) 
{ 
 ...
 static struct scsi_target *mptsas_get_starget(struct mptsas_phyinfo *phy_info ) 
{ 
 ...
 static void mptsas_set_starget(struct mptsas_phyinfo *phy_info , struct scsi_target *starget ) 
{ 
...
{ struct mptsas_portinfo_details *port_details ;
  ...
  port_details->phy_bitmask &= (u64 )(~ (1 << (int )phy_info->phy_id));
  memset((void *)(& phy_info->attached), 0, 40UL);
  ...
  if ((unsigned int )phy_info_cmp->phy_id <= 63U) {
    port_details->phy_bitmask |= (u64 )(1 << (int )phy_info_cmp->phy_id);
  }
  port_details->num_phys = (u16 )((int )port_details->num_phys + 1);
  ldv_38264: 
  j ++;
...
{ MPT_ADAPTER *ioc ;
  ...
  *((u64 *)(& smpreq->SASAddress)) = sas_address;
  ...
  if ((int )ioc->sas_mgmt.status & 1) {
    ...
    req->sense_len = 28U;
    req->resid_len = 0U;
    rsp->resid_len -= (unsigned int )smprep->ResponseDataLength;
  } else {
...
{ ConfigExtendedPageHeader_t hdr ;
  ...
  if ((unsigned int )buffer->NumPhys != 0U) {
    port_info->num_phys = (u16 )buffer->NumPhys;
  } else {
    port_info->num_phys = 1U;
  }
  tmp___0 = kcalloc((size_t )port_info->num_phys, 112UL, 208U);
  port_info->phy_info = (struct mptsas_phyinfo *)tmp___0;
  if ((unsigned long )port_info->phy_info == (unsigned long )((struct mptsas_phyinfo *)0)) {
...
{ struct mptsas_portinfo *port_info ;
  ...
  if ((unsigned int )expander_data->NumPhys != 0U) {
    port_info->num_phys = (u16 )expander_data->NumPhys;
  } else {
    port_info->num_phys = 1U;
  }
  tmp___0 = kcalloc((size_t )port_info->num_phys, 112UL, 208U);
...
{ int tmp ;
  ...
  starget_for_each_device(starget, tmp, & mptsas_reprobe_lun);
  return;
}
}
...
{ struct scsi_target *var_group1 ;
  ...
  if (ldv_s_mptsas_driver_pci_driver == 0) {
    ...
    ldv_s_mptsas_driver_pci_driver ++;
  }
  goto ldv_39476;
  case 10: ;
