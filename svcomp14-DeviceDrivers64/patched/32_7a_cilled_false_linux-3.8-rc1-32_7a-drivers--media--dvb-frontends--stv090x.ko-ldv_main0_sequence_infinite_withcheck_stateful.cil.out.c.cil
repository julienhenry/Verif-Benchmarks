...
{ struct stv090x_state *state ;
  ...
  if ((state->internal)->dev_ver <= 31U) {
    if (verbose != 0U && verbose != 0U) {
      printk("\v%s: ERROR: __VERIFIER_error(); Unsupported Cut: 0x%02x!\n", "stv090x_setup", (state->internal)->dev_ver);
    } else
    if (verbose > 1U && verbose != 0U) {
      printk("\r%s: ERROR: __VERIFIER_error(); Unsupported Cut: 0x%02x!\n", "stv090x_setup", (state->internal)->dev_ver);
    } else
    if (verbose > 2U && verbose != 0U) {
      printk("\016%s: ERROR: __VERIFIER_error(); Unsupported Cut: 0x%02x!\n", "stv090x_setup", (state->internal)->dev_ver);
    } else
    if (verbose > 3U && verbose != 0U) {
      printk("\017%s: ERROR: __VERIFIER_error(); Unsupported Cut: 0x%02x!\n", "stv090x_setup", (state->internal)->dev_ver);
    } else
    if (verbose != 0U) {
      printk("ERROR: __VERIFIER_error(); Unsupported Cut: 0x%02x!", (state->internal)->dev_ver);
    }
    goto err;
  } else
