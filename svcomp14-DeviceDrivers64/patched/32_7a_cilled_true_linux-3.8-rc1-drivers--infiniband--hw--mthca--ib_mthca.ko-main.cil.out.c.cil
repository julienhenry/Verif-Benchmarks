...
{ int tmp ;
 ...
 static unsigned int dma_set_max_seg_size(struct device *dev , unsigned int size ) 
{ 
 ...
 static int dma_get_cache_alignment(void) 
{ 
...
{ int ddr_hidden ;
  ...
  if (err == -16 && (mdev->mthca_flags & 8UL) != 0UL) {
    ...
    mdev->mthca_flags &= 0xfffffffffffffff7UL;
    err = mthca_setup_hca(mdev);
  }
  if (err != 0) {
    goto err_close;
...
int mthca_MGID_HASH(struct mthca_dev *dev , struct mthca_mailbox *mailbox , u16 *hash ) ;
 static void mthca_icm_first(struct mthca_icm *icm , struct mthca_icm_iter *iter ) 
...
static int fw_cmd_doorbell  =    0;
 static int go_bit(struct mthca_dev *dev ) 
{ unsigned int tmp ;
...
{ struct mthca_mailbox *mailbox ;
  ...
  *(inbox + 5UL) &= 4261412863U;
  *(inbox + 5UL) |= 16777216U;
  if ((dev->device_cap_flags & 262144UL) != 0UL) {
    *(inbox + 5UL) |= 939524096U;
  }
...
{ u64 mem_base ;
  ...
  (profile + (unsigned long )i)->size *= (u64 )(profile + (unsigned long )i)->num;
  ...
  if ((dev->mthca_flags & 256UL) != 0UL && (unsigned int )init_hca->log_mpt_sz > 23U) {
    ...
    dev->mthca_flags &= 0xfffffffffffffeffUL;
  }
...
{ int p ;
  ...
  *((array->page_list + (unsigned long )p)->page + ((unsigned long )index & 511UL)) = value;
  ((array->page_list + (unsigned long )p)->used) ++;
  return (0);
}
}
void mthca_array_clear(struct mthca_array *array , int index ) 
...
extern void ib_dispatch_event(struct ib_event * ) ;
 static void mthca_write64(u32 hi , u32 lo , void *dest , spinlock_t *doorbell_lock ) 
{ __u64 tmp ;
...
void mthca_qp_event(struct mthca_dev *dev , u32 qpn , enum ib_event_type event_type ) ;
 static u64 async_mask(struct mthca_dev *dev ) 
{ unsigned long long tmp ;
 ...
 static void tavor_set_eq_ci(struct mthca_dev *dev , struct mthca_eq *eq ,
                                     u32 ci ) 
{ 
 ...
 static void arbel_set_eq_ci(struct mthca_dev *dev , struct mthca_eq *eq ,
                                     u32 ci ) 
{ __u32 tmp ;
 ...
 static void set_eq_ci(struct mthca_dev *dev , struct mthca_eq *eq , u32 ci ) 
{ int tmp ;
 ...
 static void tavor_eq_req_not(struct mthca_dev *dev , int eqn ) 
{ 
 ...
 static void arbel_eq_req_not(struct mthca_dev *dev , u32 eqn_mask ) 
{ 
 ...
 static void disarm_cq(struct mthca_dev *dev , int eqn , int cqn ) 
{ int tmp ;
 ...
 static struct mthca_eqe *get_eqe(struct mthca_eq *eq , u32 entry ) 
{ unsigned long off ;
 ...
 static struct mthca_eqe *next_eqe_sw(struct mthca_eq *eq ) 
{ struct mthca_eqe *eqe ;
 ...
 static void set_eqe_hw(struct mthca_eqe *eqe ) 
{ 
...
{ int npages ;
  ...
  *(dma_list + (unsigned long )i) = t;
  ...
  if (tmp___12 != 0) {
    ...
    eq_context->logsize_usrpage |= tmp___10;
...
{ struct ib_srq  const  *__mptr ;
 ...
 static void mthca_write_db_rec(__be32 *val , __be32 *db ) 
{ 
...
void mthca_free_db(struct mthca_dev *dev , int type , int db_index ) ;
 static struct mthca_cqe *get_cqe_from_buf(struct mthca_cq_buf *buf , int entry ) 
{ 
 ...
 static struct mthca_cqe *get_cqe(struct mthca_cq *cq , int entry ) 
{ struct mthca_cqe *tmp ;
 ...
 static struct mthca_cqe *cqe_sw(struct mthca_cqe *cqe ) 
{ struct mthca_cqe *tmp ;
 ...
 static struct mthca_cqe *next_cqe_sw(struct mthca_cq *cq ) 
{ struct mthca_cqe *tmp ;
 ...
 static void set_cqe_hw(struct mthca_cqe *cqe ) 
{ 
...
{ __be32 *cqe ;
 ...
 static void update_cons_index(struct mthca_dev *dev , struct mthca_cq *cq ,
                                       int incr ) 
{ __u32 tmp ;
  int tmp___0 ;
...
{ struct mthca_cq *cq ;
 ...
 static int is_recv_cqe(struct mthca_cqe *cqe ) 
{ 
...
{ int dbd ;
 ...
 static int mthca_poll_one(struct mthca_dev *dev , struct mthca_cq *cq , struct mthca_qp **cur_qp ,
...
{ struct mthca_wq *wq ;
  ...
  if ((unsigned long )wq != (unsigned long )((struct mthca_wq *)0)) {
    if (wq->last_comp < (unsigned int )wqe_index) {
      wq->tail += (unsigned int )wqe_index - wq->last_comp;
    } else {
      wq->tail += (unsigned int )(wq->max + wqe_index) - wq->last_comp;
    }
  ...
  if (is_send != 0) {
    ...
    entry->wc_flags |= (int )((signed char )cqe->g_mlpath) < 0;
...
{ struct mthca_mailbox *mailbox ;
  ...
  if ((unsigned long )ctx != (unsigned long )((struct mthca_ucontext *)0)) {
    ...
    cq_context->logsize_usrpage |= tmp___4;
  } else {
    tmp___5 = __fswab32((__u32 )dev->driver_uar.index);
    cq_context->logsize_usrpage |= tmp___5;
  }
...
{ struct ib_fmr  const  *__mptr ;
 ...
 static void mthca_write64_raw(__be64 val , void *dest ) 
{ 
...
{ int o ;
  ...
  (*(buddy->num_free + (unsigned long )o)) --;
  goto ldv_22306;
  ldv_22305: 
  o --;
  seg <<= 1;
...
{ int tmp ;
  ...
  (*(buddy->num_free + (unsigned long )order)) --;
  seg >>= 1;
  order ++;
  ldv_22314: 
...
{ int size ;
  ...
  list_len -= chunk;
  start_index += chunk;
  buffer_list += (unsigned long )chunk;
  ldv_22425: ;
  if (list_len > 0) {
    goto ldv_22424;
  }
 ...
 static u32 tavor_hw_index_to_key(u32 ind ) 
{ 
 ...
 static u32 arbel_hw_index_to_key(u32 ind ) 
{ 
 ...
 static u32 arbel_key_to_hw_index(u32 key ) 
{ 
 ...
 static u32 hw_index_to_key(struct mthca_dev *dev , u32 ind ) 
{ u32 tmp ;
  u32 tmp___0 ;
  int tmp___1 ;
 ...
 static u32 key_to_hw_index(struct mthca_dev *dev , u32 key ) 
{ u32 tmp ;
  u32 tmp___0 ;
  int tmp___1 ;
 ...
 static u32 adjust_key(struct mthca_dev *dev , u32 key ) 
{ 
...
{ 
 ...
 static int mthca_check_fmr(struct mthca_fmr *fmr , u64 *page_list , int list_len ,
                                    u64 iova ) 
{ int i ;
  int page_mask ;
...
enum ib_rate mthca_rate_to_ib(struct mthca_dev *dev , u8 mthca_rate , u8 port ) ;
 static void mthca_set_data_seg(struct mthca_data_seg *dseg , struct ib_sge *sg ) 
{ __u32 tmp ;
  __u32 tmp___0 ;
  __u64 tmp___1 ;
 ...
 static void mthca_set_data_seg_inval(struct mthca_data_seg *dseg ) 
{ 
...
{ u8 dest_rd_atomic ;
 ...
 static enum ib_qp_state to_ib_qp_state(int mthca_state ) 
{ 
 ...
 static enum ib_mig_state to_ib_mig_state(int mthca_mig_state ) 
{ 
...
{ struct mthca_dev *dev ;
  ...
  qp_context->flags |= 65536U;
  if ((attr_mask & 262144) == 0) {
    qp_context->flags |= 1572864U;
  } else {
    qp_param->opt_param_mask |= 262144U;
    switch ((unsigned int )attr->path_mig_state) {
    case 0: 
    qp_context->flags |= 1572864U;
    goto ldv_26750;
    case 1: 
    qp_context->flags |= 524288U;
    goto ldv_26750;
    case 2: 
  ...
  if ((unsigned int )qp->transport == 5U) {
    ...
    qp_context->pri_path.port_pkey |= tmp___14;
  } else
  if ((attr_mask & 32) != 0) {
    tmp___15 = __fswab32((__u32 )((int )attr->port_num << 24));
    qp_context->pri_path.port_pkey |= tmp___15;
    qp_param->opt_param_mask |= 524288U;
  }
  if ((attr_mask & 16) != 0) {
    tmp___16 = __fswab32((__u32 )attr->pkey_index);
    qp_context->pri_path.port_pkey |= tmp___16;
    qp_param->opt_param_mask |= 268435456U;
  }
  if ((attr_mask & 2048) != 0) {
    tmp___17 = (int )((u8 )attr->rnr_retry) << 5U;
    qp_context->pri_path.rnr_retry = tmp___17;
    qp_context->alt_path.rnr_retry = tmp___17;
    qp_param->opt_param_mask |= 10485760U;
  }
  if ((attr_mask & 128) != 0) {
    if ((attr_mask & 32) != 0) {
    ...
    qp_param->opt_param_mask |= 2147483648U;
  }
  if (((unsigned int )ibqp->qp_type == 2U && (unsigned int )cur_state == 1U) && (unsigned int )new_state == 2U) {
    ...
    if (tmp___22 != 0) {
      ...
      qp_context->tavor_sched_queue |= tmp___21;
    }
    qp_param->opt_param_mask |= 256U;
  }
  if ((attr_mask & 512) != 0) {
    qp_context->pri_path.ackto = (int )((u8 )attr->timeout) << 3U;
    qp_param->opt_param_mask |= 4194304U;
  }
  if ((attr_mask & 16384) != 0) {
    ...
    qp_context->alt_path.port_pkey |= tmp___24;
    qp_context->alt_path.ackto = (int )((u8 )attr->alt_timeout) << 3U;
    qp_param->opt_param_mask |= 16777216U;
  }
  ...
  if ((unsigned int )qp->sq_policy == 0U) {
    qp_context->params1 |= 134217728U;
  }
  if ((attr_mask & 1024) != 0) {
    tmp___28 = __fswab32((__u32 )((int )attr->retry_cnt << 16));
    qp_context->params1 |= tmp___28;
    qp_param->opt_param_mask |= 1048576U;
  }
  if ((attr_mask & 8192) != 0) {
    if ((unsigned int )((unsigned char )attr->max_rd_atomic) != 0U) {
      qp_context->params1 |= 10485760U;
      tmp___29 = fls((int )attr->max_rd_atomic + -1);
      tmp___30 = __fswab32((__u32 )(tmp___29 << 21));
      qp_context->params1 |= tmp___30;
    }
    qp_param->opt_param_mask |= 65536U;
  }
  if ((attr_mask & 65536) != 0) {
  ...
  if ((attr_mask & 131072) != 0) {
    if ((unsigned int )((unsigned char )attr->max_dest_rd_atomic) != 0U) {
      ...
      qp_context->params2 |= tmp___38;
    }
    qp_param->opt_param_mask |= 131072U;
  }
  if ((attr_mask & 131080) != 0) {
    tmp___39 = get_hw_access_flags(qp, attr, attr_mask);
    qp_context->params2 |= tmp___39;
    qp_param->opt_param_mask |= 234881024U;
  }
  qp_context->params2 |= 134217728U;
  if ((unsigned long )ibqp->srq != (unsigned long )((struct ib_srq *)0)) {
    qp_context->params2 |= 268435456U;
  }
  if ((attr_mask & 32768) != 0) {
    tmp___40 = __fswab32((__u32 )((int )attr->min_rnr_timer << 24));
    qp_context->rnr_nextrecvpsn |= tmp___40;
    qp_param->opt_param_mask |= 1073741824U;
  }
  if ((attr_mask & 4096) != 0) {
    tmp___41 = __fswab32(attr->rq_psn);
    qp_context->rnr_nextrecvpsn |= tmp___41;
  }
  ...
  if ((attr_mask & 64) != 0) {
    ...
    qp_param->opt_param_mask |= 536870912U;
  }
  if ((unsigned long )ibqp->srq != (unsigned long )((struct ib_srq *)0)) {
...
{ int max_data_size ;
 ...
 static int mthca_max_inline_data(struct mthca_pd *pd , int max_data_size ) 
{ int tmp ;
...
{ u32 mqpn ;
 ...
 static int get_qp_refcount(struct mthca_dev *dev , struct mthca_qp *qp ) 
{ int c ;
...
{ int header_size ;
 ...
 static int mthca_wq_overflow(struct mthca_wq *wq , int nreq , struct ib_cq *ib_cq ) 
{ unsigned int cur ;
  struct mthca_cq *cq ;
  long tmp ;
 ...
 static void set_raddr_seg(struct mthca_raddr_seg *rseg , u64 remote_addr ,
                                   u32 rkey ) 
{ __u64 tmp ;
  __u32 tmp___0 ;
 ...
 static void set_atomic_seg(struct mthca_atomic_seg *aseg , struct ib_send_wr *wr ) 
{ __u64 tmp ;
  __u64 tmp___0 ;
  __u64 tmp___1 ;
...
{ u32 index ;
  ...
  if ((int )ah_attr->ah_flags & 1) {
    ...
    av->sl_tclass_flowlabel |= tmp___8;
    __len = 16UL;
    if (__len > 63UL) {
...
{ struct mthca_mr *mr ;
  ...
  shift = (int )tmp___0;
  buffer_list->size += buffer_list->addr & ((1ULL << shift) - 1ULL);
  buffer_list->addr &= 0xffffffffffffffffULL << shift;
...
{ int ret ;
  ...
  if ((dev->mthca_flags & 4UL) != 0UL) {
    ...
    dev->ib_dev.uverbs_cmd_mask |= 64424509440ULL;
    tmp = mthca_is_memfree(dev);
    if (tmp != 0) {
...
{ int tmp___0 ;
 ...
 static int bitmap_full(unsigned long const   *src , int nbits ) 
{ int tmp___0 ;
...
{ int tmp ;
  ...
  (db_tab->page[index / 512].refcount) --;
...
{ int logsize ;
  ...
  if ((unsigned long )pd->ibpd.uobject != (unsigned long )((struct ib_uobject *)0)) {
    ...
    context->logstride_usrpage |= tmp___4;
  } else {
    tmp___5 = __fswab32((__u32 )dev->driver_uar.index);
    context->logstride_usrpage |= tmp___5;
  }
...
{ struct mthca_mailbox *mailbox ;
 ...
 static int get_srq_refcount(struct mthca_dev *dev , struct mthca_srq *srq ) 
{ int c ;
