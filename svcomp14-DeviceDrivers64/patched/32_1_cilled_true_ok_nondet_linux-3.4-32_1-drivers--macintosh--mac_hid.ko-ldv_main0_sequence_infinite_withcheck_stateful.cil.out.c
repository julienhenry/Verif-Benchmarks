...
{ int err ;
  ...
  *((unsigned long *)__cil_tmp30) = 1UL | __cil_tmp31;
  tmp = (int )input_register_device(mac_hid_emumouse_dev);
  err = tmp;
  if (err) {
...
{ int err ;
  ...
  tmp = (int )input_register_handler(& mac_hid_emumouse_handler);
  err = tmp;
  if (err) {
...
{ int *valp ;
  ...
  old_val = *valp;
  tmp = (int )mutex_lock_killable(& mac_hid_emumouse_mutex);
  rc = tmp;
  if (rc) {
    return (rc);
  }
    ...
    if (tmp___0 == 2) {
      ...
      if (0) {
        ...
        if (ldv_s_mac_hid_emumouse_handler_input_handler == 0) {
          ...
          ldv_s_mac_hid_emumouse_handler_input_handler ++;
        }
        goto switch_break;
        case_1: 
