...
static struct attribute ttm_mem_emer  =    {"emergency_memory",
    420U, (_Bool)0, 0, {{{(char)0}, {(char)0}, {(char)0}, {(char)0}, {(char)0}, {(char)0},
                         {(char)0}, {(char)0}}}};
static struct attribute ttm_mem_max  =    {"available_memory", 420U, (_Bool)0, 0, {{{(char)0}, {(char)0}, {(char)0}, {(char)0},
                                             {(char)0}, {(char)0}, {(char)0}, {(char)0}}}};
static struct attribute ttm_mem_swap  =    {"swap_limit",
    420U, (_Bool)0, 0, {{{(char)0}, {(char)0}, {(char)0}, {(char)0}, {(char)0}, {(char)0},
                         {(char)0}, {(char)0}}}};
...
extern struct page *shmem_read_mapping_page_gfp(struct address_space * , unsigned long  ,
                                                gfp_t  ) ;
 static struct page *shmem_read_mapping_page(struct address_space *mapping ,
                                                     unsigned long index ) 
{ gfp_t tmp ;
  struct page *tmp___0 ;
...
extern void drm_clflush_pages(struct page ** , unsigned long  ) ;
 static void *drm_calloc_large(size_t nmemb , size_t size ) 
{ void *tmp ;
 ...
 static void drm_free_large(void *ptr ) 
{ int tmp ;
...
{ void *tmp ;
 ...
 static int ttm_tt_set_page_caching(struct page *p , enum ttm_caching_state c_old ,
...
{ int tmp ;
 ...
 static void ttm_mem_init_shrink(struct ttm_mem_shrink *shrink , int (*func)(struct ttm_mem_shrink * ) ) 
{ 
 ...
 static int ttm_mem_register_shrink(struct ttm_mem_global *glob , struct ttm_mem_shrink *shrink ) 
{ 
 ...
 static void ttm_mem_unregister_shrink(struct ttm_mem_global *glob , struct ttm_mem_shrink *shrink ) 
{ long tmp ;

  {
  spin_lock(& glob->lock);
  tmp = ldv__builtin_expect((unsigned long )glob->shrink != (unsigned long )shrink,
                            0L);
  if (tmp != 0L) {
...
extern struct drm_mm_node *drm_mm_get_block_generic(struct drm_mm_node * , unsigned long  ,
                                                    ...
                                                    int  ) ;
 static struct drm_mm_node *drm_mm_get_block_atomic(struct drm_mm_node *parent ,
...
extern struct drm_mm_node *drm_mm_search_free_generic(struct drm_mm  const  * , unsigned long  ,
                                                      unsigned int  , unsigned long  ,
                                                      bool  ) ;
 static struct drm_mm_node *drm_mm_search_free(struct drm_mm  const  *mm ,
                                                       unsigned long size , unsigned int alignment ,
                                                       bool best_match ) 
{ struct drm_mm_node *tmp ;
...
extern int drm_mm_pre_get(struct drm_mm * ) ;
 static uint32_t ttm_flag_masked(uint32_t *old , uint32_t new , uint32_t mask ) 
{ 
...
static struct attribute ttm_bo_count  =    {"bo_count", 292U, (_Bool)0, 0, {{{(char)0}, {(char)0}, {(char)0}, {(char)0}, {(char)0},
 ...
 static int ttm_mem_type_from_flags(uint32_t flags , uint32_t *mem_type ) 
{ int i ;
...
static struct kobj_type ttm_bo_glob_kobj_type  =    {& ttm_bo_global_kobj_release, & ttm_bo_global_ops, (struct attribute **)(& ttm_bo_global_attrs),
    0, 0};
 static uint32_t ttm_bo_type_flags(unsigned int type ) 
{ 
...
void ttm_bo_list_ref_sub(struct ttm_buffer_object *bo , int count , bool never_free ) 
{ void (*tmp)(struct kref *list_kref ) ;

  {
  if ((int )never_free) {
    tmp = & ttm_bo_ref_bug;
  } else {
    tmp = & ttm_bo_release_list;
  }
  kref_sub(& bo->list_kref, (unsigned int )count, tmp);
  return;
}
}
int ttm_bo_reserve(struct ttm_buffer_object *bo , bool interruptible , bool no_wait ,
...
void ldv_mutex_lock_53(struct mutex *ldv_func_arg1 ) ;
 static int kref_get_unless_zero(struct kref *kref ) 
{ int tmp ;
...
{ struct ttm_buffer_object *bo ;
  ...
  virtual += (unsigned long )page_offset;
  if ((int )write) {
...
{ struct vm_area_struct *var_group1 ;
  ...
  if (ldv_s_ttm_bo_vm_ops_vm_operations_struct == 0) {
    ...
    ldv_s_ttm_bo_vm_ops_vm_operations_struct ++;
  }
  goto ldv_22118;
  case 1: ;
...
void ttm_object_device_release(struct ttm_object_device **p_tdev ) ;
 static struct ttm_object_file *ttm_object_file_ref(struct ttm_object_file *tfile ) 
{ 
...
{ struct ttm_object_file *tfile ;
 ...
 static void ttm_object_file_unref(struct ttm_object_file **p_tfile ) 
{ struct ttm_object_file *tfile ;
...
void ldv_mutex_lock_93(struct mutex *ldv_func_arg1 ) ;
 static struct thread_info *current_thread_info___3(void) 
{ struct thread_info *ti ;
  unsigned long pfo_ret__ ;
...
{ struct page *tmp ;
 ...
 static void pagefault_disable___1(void) 
{ struct thread_info *tmp ;
 ...
 static void pagefault_enable___1(void) 
{ struct thread_info *tmp ;
...
extern int set_pages_array_wb(struct page ** , int  ) ;
 static void *kmap_atomic___1(struct page *page ) 
{ void *tmp ;
 ...
 static void __kunmap_atomic___1(void *addr ) 
{ 
...
static struct attribute ttm_page_pool_max  =    {"pool_max_size", 420U, (_Bool)0, 0, {{{(char)0}, {(char)0}, {(char)0}, {(char)0},
                                          {(char)0}, {(char)0}, {(char)0}, {(char)0}}}};
static struct attribute ttm_page_pool_small  =    {"pool_small_allocation",
    420U, (_Bool)0, 0, {{{(char)0}, {(char)0}, {(char)0}, {(char)0}, {(char)0}, {(char)0},
                         {(char)0}, {(char)0}}}};
...
{ 
  ...
  pool->nfrees += (unsigned long )freed_pages;
  return;
}
}
...
extern struct drm_mm_node *drm_mm_get_block_range_generic(struct drm_mm_node * , unsigned long  ,
                                                          ...
                                                          unsigned long  , unsigned long  ,
                                                          int  ) ;
 static struct drm_mm_node *drm_mm_get_block_atomic_range(struct drm_mm_node *parent ,
                                                                  unsigned long size ,
...
extern struct drm_mm_node *drm_mm_search_free_in_range_generic(struct drm_mm  const  * ,
                                                               ...
                                                               unsigned long  , unsigned long  ,
                                                               unsigned long  , bool  ) ;
 static struct drm_mm_node *drm_mm_search_free_in_range(struct drm_mm  const  *mm ,
                                                                unsigned long size ,
                                                                unsigned int alignment ,
                                                                unsigned long start ,
                                                                unsigned long end ,
...
static struct attribute ttm_page_pool_max___0  =    {"pool_max_size", 420U, (_Bool)0, 0, {{{(char)0}, {(char)0}, {(char)0}, {(char)0},
                                          {(char)0}, {(char)0}, {(char)0}, {(char)0}}}};
static struct attribute ttm_page_pool_small___0  =    {"pool_small_allocation",
    420U, (_Bool)0, 0, {{{(char)0}, {(char)0}, {(char)0}, {(char)0}, {(char)0}, {(char)0},
                         {(char)0}, {(char)0}}}};
...
{ struct ttm_tt *ttm ;
  ...
  *(ttm->pages + (unsigned long )count) = d_page->p;
  ...
  if ((int )is_cached) {
    pool->nfrees += (unsigned long )count;
  } else {
    pool->npages_free += count;
...
{ atomic_t start_pool ;
  ...
  pool_offset %= _manager___0->npools;
  __mptr = (struct list_head  const  *)_manager___0->pools.next;
  p = (struct device_pools *)__mptr;
  goto ldv_28418;
  ldv_28417: ;
