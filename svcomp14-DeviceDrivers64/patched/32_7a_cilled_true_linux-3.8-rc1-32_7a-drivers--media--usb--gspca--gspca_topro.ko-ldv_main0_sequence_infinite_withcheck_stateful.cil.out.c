...
{ struct sd *sd ;
  ...
  if ((unsigned int )sd->sensor == 0U) {
    if ((gspca_dev->ldv_27677.gain)->cur.val != 0) {
      old = (gspca_dev->ldv_27677.gain)->cur.val;
    } else {
      old = 1;
    }
...
{ struct sd *sd ;
  ...
  if ((unsigned int )sd->bridge == 0U) {
    gspca_dev->cam.mode_framerates = (struct framerates  const  *)(& framerates);
  } else {
    gspca_dev->cam.mode_framerates = (struct framerates  const  *)(& framerates_6810);
  }
  sd->framerate = 30U;
  return (0);
}
...
{ struct sd *sd ;
  ...
  if ((unsigned int )sd->sensor == 1U) {
    ...
    sd->jpegqual = v4l2_ctrl_new_std(hdl, & sd_ctrl_ops, 10291459U, 0, 15, 1U, tmp);
  }
  if (hdl->error != 0) {
