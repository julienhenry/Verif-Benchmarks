...
void ubi_dump_av(struct ubi_ainf_volume  const  *av ) ;
 static int ubi_dbg_chk_gen(struct ubi_device  const  *ubi ) 
{ 
...
int ubi_io_write_vid_hdr(struct ubi_device *ubi , int pnum , struct ubi_vid_hdr *vid_hdr ) ;
 static struct ubi_vid_hdr *ubi_zalloc_vid_hdr(struct ubi_device  const  *ubi ,
                                                       gfp_t gfp_flags ) 
{ void *vid_hdr ;
 ...
 static void ubi_free_vid_hdr(struct ubi_device  const  *ubi , struct ubi_vid_hdr *vid_hdr ) 
{ void *p ;
 ...
 static int ubi_io_read_data(struct ubi_device  const  *ubi , void *buf ,
...
{ struct task_struct *tmp ;
 ...
 static int ubi_io_write_data(struct ubi_device *ubi , void const   *buf ,
                                      int pnum , int offset , int len ) 
{ struct task_struct *tmp ;
  long tmp___0 ;
  int tmp___1 ;
 ...
 static int vol_id2idx(struct ubi_device  const  *ubi , int vol_id ) 
{ 
...
{ int i ;
  ...
  if ((unsigned int )((unsigned char )(vtbl + (unsigned long )i)->vol_type) == 1U) {
    vol->vol_type = 3;
  } else {
    vol->vol_type = 4;
  }
  ...
  vol->used_bytes += (long long )av->last_data_size;
...
{ int lnum ;
  ...
  if (offs != 0) {
    ...
    vol->upd_received += (long long )len;
    count -= len;
...
{ int err ;
  ...
  vol->upd_received += (long long )count;
  if (vol->upd_received == vol->upd_bytes) {
...
{ uint32_t __base ;
 ...
 static int mtd_can_have_bb(struct mtd_info  const  *mtd ) 
{ 
...
static spinlock_t ubi_devices_lock  =    {{{{{0U}},
      3735899821U, 4294967295U, 0xffffffffffffffffUL, {0, {0, 0}, "ubi_devices_lock",
                                                       0, 0UL}}}};
...
{ struct ubi_device *ubi ;
  ...
  _min2 = 256;
  if (_min1 < _min2) {
    ubi->fm_pool.max_size = _min1;
  } else {
    ubi->fm_pool.max_size = _min2;
  }
...
{ struct ubi_volume_desc *desc ;
  ...
  if (count < (size_t )tbuf_size) {
    tmp___4 = count;
  } else {
    tmp___4 = (size_t )tbuf_size;
  }
  len = (int )tmp___4;
...
{ struct inode *var_group1 ;
  ...
  if (ldv_s_ubi_vol_cdev_operations_file_operations == 0) {
    ...
    ldv_s_ubi_vol_cdev_operations_file_operations ++;
  }
  goto ldv_26120;
  case 1: ;
...
int ubi_io_read_vid_hdr(struct ubi_device *ubi , int pnum , struct ubi_vid_hdr *vid_hdr ,
                        int verbose ) ;
 static void ubi_move_aeb_to_list(struct ubi_ainf_volume *av , struct ubi_ainf_peb *aeb ,
                                          struct list_head *list ) 
{ 
 ...
 static int idx2vol_id(struct ubi_device  const  *ubi , int idx ) 
{ 
...
int ubi_self_check_all_ff(struct ubi_device *ubi , int pnum , int offset , int len ) ;
 static int ubi_dbg_is_bitflip(struct ubi_device  const  *ubi ) 
{ u32 tmp ;
 ...
 static int ubi_dbg_is_write_failure(struct ubi_device  const  *ubi ) 
{ u32 tmp ;
 ...
 static int ubi_dbg_is_erase_failure(struct ubi_device  const  *ubi ) 
{ u32 tmp ;
 ...
 static int ubi_dbg_chk_io(struct ubi_device  const  *ubi ) 
{ 
...
extern bool kthread_should_stop(void) ;
 static int ubi_dbg_is_bgt_disabled(struct ubi_device  const  *ubi ) 
{ 
...
{ struct ubi_ainf_volume *av ;
  ...
  if ((unsigned int )((unsigned char )vid_hdr->vol_type) == 1U) {
    av->vol_type = 3;
  } else {
    av->vol_type = 4;
  }
...
{ long long ec ;
  ...
  adjust_mean_ec: ;
  if (ec_err == 0) {
    ai->ec_sum += (unsigned long long )ec;
    (ai->ec_count) ++;
...
{ int pnum ;
  ...
  if ((unsigned int )vidh->vol_type == 1U) {
    vol_type = 3;
  } else {
    vol_type = 4;
  }
