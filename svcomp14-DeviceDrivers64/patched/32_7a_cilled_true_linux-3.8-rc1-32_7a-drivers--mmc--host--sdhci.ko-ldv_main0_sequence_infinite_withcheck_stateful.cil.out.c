...
extern void led_classdev_unregister(struct led_classdev * ) ;
 static bool mmc_op_multi(u32 opcode ) 
{ 
...
{ 
  ...
  if ((host->flags & 2048) != 0) {
    ...
    host->flags &= -33;
    if ((host->quirks & 2097152U) != 0U) {
      (host->mmc)->max_blk_count = 1U;
    } else {
      (host->mmc)->max_blk_count = 65535U;
    }
  }
...
{ struct mmc_data *data ;
  ...
  if ((host->flags & 4) != 0) {
    if ((host->flags & 2) != 0) {
      ...
      dma_unmap_sg_attrs((host->mmc)->parent, data->sg, (int )data->sg_len, tmp___0,
                         0);
    }
  }
  if (data->error != 0U) {
...
{ unsigned long flags ;
  ...
  spin_unlock_irqrestore(& host->lock, flags);
  if ((host->quirks & 65536U) != 0U) {
    tmp___2 = is_readonly == 0;
  } else {
    tmp___2 = is_readonly;
  }
  return (tmp___2);
...
{ irqreturn_t result ;
  ...
  if ((intmask & 192U) != 0U) {
    ...
    sdhci_mask_irqs(host, tmp___5);
    if (present != 0U) {
      tmp___6 = 128U;
    } else {
      tmp___6 = 64U;
    }
    sdhci_unmask_irqs(host, tmp___6);
