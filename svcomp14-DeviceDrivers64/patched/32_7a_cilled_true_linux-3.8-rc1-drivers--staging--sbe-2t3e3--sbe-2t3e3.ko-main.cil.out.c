...
int setup_device(struct net_device *dev , struct channel *sc ) ;
 static int has_two_ports(struct pci_dev *pdev ) 
{ 
...
extern int hdlc_ioctl(struct net_device * , struct ifreq * , int  ) ;
 static struct hdlc_device *dev_to_hdlc(struct net_device *dev ) 
{ void *tmp ;
...
void exar7300_init(struct channel *sc ) ;
 static void dc_write(unsigned long addr , u32 reg , u32 val ) 
{ 
 ...
 static void cpld_write(struct channel *channel , unsigned int reg , u32 val ) 
{ unsigned long flags ;
  raw_spinlock_t *tmp ;
...
{ struct channel *sc ;
  ...
  if (skb2->len != 0U) {
    ...
    (sc->ether.tx_ring + (unsigned long )current_write)->tdes1 &= 50331648U;
    ...
    if (sc->ether.tx_ring_current_write == current_write) {
      (sc->ether.tx_ring + (unsigned long )current_write)->tdes1 |= 536870912U;
    } else {
  ...
  (sc->ether.tx_ring + (unsigned long )last_write)->tdes1 |= 3221225472U;
  (sc->ether.tx_ring + (unsigned long )sc->ether.tx_ring_current_write)->tdes0 |= 2147483648U;
...
void exar7300_unipolar_onoff(struct channel *sc , u32 mode ) ;
 static void cpld_set_bit(struct channel *channel , unsigned int reg , u32 bit ) 
{ unsigned long flags ;
  raw_spinlock_t *tmp ;
  u32 tmp___0 ;
 ...
 static void cpld_clear_bit(struct channel *channel , unsigned int reg , u32 bit ) 
{ unsigned long flags ;
  raw_spinlock_t *tmp ;
  u32 tmp___0 ;
...
extern int netif_rx(struct sk_buff * ) ;
 static __be16 hdlc_type_trans(struct sk_buff *skb , struct net_device *dev ) 
{ hdlc_device *hdlc ;
  struct hdlc_device *tmp ;
  __be16 tmp___0 ;
...
void exar7250_write(struct channel *channel , u32 reg , u32 val ) ;
 static u32 dc_read(unsigned long addr , u32 reg ) 
{ unsigned int tmp ;
 ...
 static void dc_set_bits(unsigned long addr , u32 reg , u32 bits ) 
{ u32 tmp ;
 ...
 static void dc_clear_bits(unsigned long addr , u32 reg , u32 bits ) 
{ u32 tmp ;
...
{ u32 current_read ;
  ...
  if ((unsigned int )sc->p.loopback != 1U && (unsigned int )sc->s.OOF != 0U) {
    ...
    current_desc->rdes1 &= 50331648U;
    current_desc->rdes1 |= 1600U;
  ...
  if (rcv_len > 1600U) {
    ...
    if ((unsigned long )m2 != (unsigned long )((struct sk_buff *)0)) {
      ...
      (sc->s.in_packets) ++;
      sc->s.in_bytes += (u_int64_t )rcv_len;
      m->dev = sc->dev;
      skb_put(m, rcv_len);
...
{ u32 current_read ;
  ...
  if ((unsigned long )sc->ether.tx_data[current_read] != (unsigned long )((struct sk_buff *)0)) {
    (sc->s.out_packets) ++;
    sc->s.out_bytes += (u_int64_t )(sc->ether.tx_data[current_read])->len;
...
{ u32 i ;
  ...
  ldv_39484: 
  (sc->ether.rx_ring + 63UL)->rdes1 |= 33554432U;
  ...
  ldv_39487: 
  (sc->ether.tx_ring + 255UL)->tdes1 |= 33554432U;
