...
{ struct usb_device *dev ;
  ...
  input_dev->evbit[0] |= (1UL << 1) | (1UL << 3);
  input_dev->keybit[4] |= (1UL << 16) | (1UL << 17);
  input_dev->keybit[5] |= 1UL | (1UL << 10);
  ...
  (kbtab->irq)->transfer_flags |= 4U;
  tmp___11 = (int )input_register_device(kbtab->dev);
  tmp = tmp___11;
  error = tmp;
  if (error) {
    goto fail3;
