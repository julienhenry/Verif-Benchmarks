...
{ struct mxser_port *info ;
  ...
  if ((tty->termios.c_iflag & 16U) != 0U) {
    info->read_status_mask |= 12;
  }
  if ((tty->termios.c_iflag & 2U) != 0U || (tty->termios.c_iflag & 8U) != 0U) {
    info->read_status_mask |= 16;
  }
  info->ignore_status_mask = 0;
  if ((int )tty->termios.c_iflag & 1) {
    info->ignore_status_mask |= 16;
    info->read_status_mask |= 16;
    if ((tty->termios.c_iflag & 4U) != 0U) {
      info->ignore_status_mask |= 14;
      info->read_status_mask |= 14;
    }
  }
  if ((info->board)->chip_flag != 0) {
...
{ struct mxser_port *info ;
  ...
  return (((((tmp___1 | tmp___2) | tmp___3) | tmp___4) | tmp___5) | tmp___6);
}
}
...
{ unsigned char ch ;
  ...
  *status = (int )tmp___1;
  ...
  ldv_30649: ;
  end_intr: 
  mxvar_log.rxcnt[tty->index] += (unsigned long )cnt;
  port->mon_data.rxcnt += (unsigned long )cnt;
  port->mon_data.up_rxcnt += (unsigned long )cnt;
...
{ int count ;
  ...
  ldv_30658: 
  mxvar_log.txcnt[tty->index] += (unsigned long )(cnt - port->xmit_cnt);
  port->mon_data.txcnt += (unsigned long )(cnt - port->xmit_cnt);
  port->mon_data.up_txcnt += (unsigned long )(cnt - port->xmit_cnt);
  port->icount.tx += (__u32 )(cnt - port->xmit_cnt);
...
{ struct mxser_board *brd ;
  ...
  goto ldv_30795;
  ldv_30794: 
  brd->vector_mask |= (unsigned long )(1 << (int )i);
  brd->ports[i].baud_base = 921600;
  i ++;
  ldv_30795: ;
  if ((unsigned int )(brd->info)->nports > i) {
...
{ struct tty_struct *var_group1 ;
  ...
  if (ldv_s_mxser_ops_tty_operations == 0) {
    ...
    ldv_s_mxser_ops_tty_operations ++;
  }
  goto ldv_30885;
  case 1: ;
