...
int cx18_init_on_first_open(struct cx18 *cx ) ;
 static u32 cx18_readl(struct cx18 *cx , void const   *addr ) 
{ unsigned int tmp ;
...
{ int i ;
  ...
  if (i == 2) {
    ...
    cx->stream_buf_size[i] -= cx->stream_buf_size[i] % 34560;
...
static struct cx18_card_tuner_i2c cx18_i2c_std  =    {{65534U, (unsigned short)0}, {67U, 65534U, (unsigned short)0}, {97U, 96U, 65534U,
                                                                    (unsigned short)0}};
static struct cx18_card_tuner_i2c cx18_i2c_nxp  =    {{65534U,
     (unsigned short)0}, {66U, 67U, 65534U}, {97U, 96U, 65534U, (unsigned short)0}};
...
static struct cx18_card_pci_info  const  cx18_pci_h900[2U]  = {      {23418U,
      6235U, 57600U}, 
        {0U, 0U, 0U}};
...
static struct cx18_card_pci_info  const  cx18_pci_mpc718[2U]  = {      {23418U,
      4779U, 1816U}, 
        {0U, 0U, 0U}};
...
static struct cx18_card_pci_info  const  cx18_pci_gotview_dvd3[2U]  = {      {23418U,
      22612U, 13123U}, 
        {0U, 0U, 0U}};
...
static struct cx18_card_pci_info  const  cx18_pci_cnxt_raptor_pal[2U]  = {      {23418U,
      5361U, 9U}, 
        {0U, 0U, 0U}};
...
static struct cx18_card_pci_info  const  cx18_pci_toshiba_qosmio_dvbt[2U]  = {      {23418U,
      4473U, 272U}, 
        {0U, 0U, 0U}};
...
static struct cx18_card_pci_info  const  cx18_pci_leadtek_dvr3100h[2U]  = {      {23418U,
      4221U, 26256U}, 
        {0U, 0U, 0U}};
...
extern struct v4l2_subdev *v4l2_i2c_new_subdev(struct v4l2_device * , struct i2c_adapter * ,
 ...
 static void cx18_writel_noretry(struct cx18 *cx , u32 val , void *addr ) 
{ 
 ...
 static void cx18_writel(struct cx18 *cx , u32 val , void *addr ) 
{ int i ;
  u32 tmp ;
 ...
 static void cx18_writel_expect(struct cx18 *cx , u32 val , void *addr , u32 eval ,
                                        u32 mask ) 
{ int i ;
  u32 r ;
...
{ 
 ...
 static void cx18_write_reg_expect(struct cx18 *cx , u32 val , u32 reg , u32 eval ,
                                           u32 mask ) 
{ 
...
int cx18_vapi_result(struct cx18 *cx , u32 *data , u32 cmd , int args  , ...) ;
 static u32 cx18_raw_readl(struct cx18 *cx , void const   *addr ) 
{ unsigned int tmp ;
 ...
 static void cx18_raw_writel_noretry(struct cx18 *cx , u32 val , void *addr ) 
{ 
 ...
 static void cx18_raw_writel(struct cx18 *cx , u32 val , void *addr ) 
{ int i ;
  u32 tmp ;
...
extern long schedule_timeout_uninterruptible(long  ) ;
 static void v4l2_set_subdevdata(struct v4l2_subdev *sd , void *p ) 
{ 
...
extern int v4l2_device_register_subdev(struct v4l2_device * , struct v4l2_subdev * ) ;
 static void cx18_writel_expect___1(struct cx18 *cx , u32 val , void *addr ,
                                            u32 eval , u32 mask ) 
{ int i ;
  u32 r ;
 ...
 static void cx18_write_reg_expect___1(struct cx18 *cx , u32 val , u32 reg ,
                                               u32 eval , u32 mask ) 
{ 
...
{ u32 tmp ;
  ...
  if ((int )(cx->card)->tuners[0].tuner == 71) {
    cx->gpio_dir |= (u32 )(1 << (int )(cx->card)->xceive_pin);
    cx->gpio_val |= (u32 )(1 << (int )(cx->card)->xceive_pin);
  }
  if (cx->gpio_dir == 0U) {
...
{ 
 ...
 static int list_is_singular(struct list_head  const  *head ) 
{ int tmp ;
...
{ struct device *tmp ;
 ...
 static void cx18_buf_sync_for_cpu(struct cx18_stream *s , struct cx18_buffer *buf ) 
{ 
...
struct cx18_queue *_cx18_enqueue(struct cx18_stream *s , struct cx18_mdl *mdl , struct cx18_queue *q ,
                                 int to_front ) ;
 static struct cx18_queue *cx18_enqueue(struct cx18_stream *s , struct cx18_mdl *mdl ,
                                                struct cx18_queue *q ) 
{ struct cx18_queue *tmp ;
 ...
 static struct cx18_queue *cx18_push(struct cx18_stream *s , struct cx18_mdl *mdl ,
                                             struct cx18_queue *q ) 
{ struct cx18_queue *tmp ;
...
void cx18_stream_free(struct cx18_stream *s ) ;
 static void cx18_writel___1(struct cx18 *cx , u32 val , void *addr ) 
{ int i ;
  u32 tmp ;
...
{ struct cx18_mdl *mdl ;
  ...
  if (tmp == 0) {
    ...
    q->bytesused += mdl->readpos - mdl->bytesused;
    mdl->skipped = 0U;
    atomic_dec(& q->depth);
  }
  spin_unlock(& q->lock);
...
{ struct cx18_buffer *buf ;
 ...
 static void cx18_mdl_update_bufs_for_cpu(struct cx18_stream *s , struct cx18_mdl *mdl ) 
{ struct cx18_buffer *buf ;
  struct list_head  const  *__mptr ;
  int tmp ;
...
extern int __video_register_device(struct video_device * , int  , int  , int  , struct module * ) ;
 static int video_register_device_no_warn(struct video_device *vdev , int type ,
                                                  int nr ) 
{ int tmp ;
...
static unsigned int const   vbi_hblank_samples_50Hz  =    284U;
 static int cx18_raw_vbi(struct cx18  const  *cx ) 
{ 
...
void cx18_vb_timeout(unsigned long data ) ;
 static void cx18_buf_sync_for_device(struct cx18_stream *s , struct cx18_buffer *buf ) 
{ 
 ...
 static void cx18_mdl_sync_for_device(struct cx18_stream *s , struct cx18_mdl *mdl ) 
{ struct list_head  const  *__mptr ;
  int tmp ;
...
{ 
 ...
 static void cx18_stream_put_mdl_fw(struct cx18_stream *s , struct cx18_mdl *mdl ) 
{ 
...
{ struct cx18 *cx ;
  ...
  *stop = 0;
  ...
  if (s->type == 0 && (unsigned long )(& cx->vbi.sliced_mpeg_buf) != (unsigned long )buf) {
    cx->mpg_data_received += (unsigned long long )len;
  }
  return (len);
}
}
...
{ struct cx18 *cx ;
  ...
  if (mdl->readpos == mdl->bytesused) {
    ...
    cx->vbi_data_inserted += (u64 )mdl->bytesused;
  }
  if (rc < 0) {
    return ((ssize_t )rc);
  }
...
static unsigned int const   vbi_active_samples___1  =    1444U;
 static void cx18_writel___3(struct cx18 *cx , u32 val , void *addr ) 
{ int i ;
  u32 tmp ;
...
extern void *videobuf_to_vmalloc(struct videobuf_buffer * ) ;
 static void cx18_writel___4(struct cx18 *cx , u32 val , void *addr ) 
{ int i ;
  u32 tmp ;
 ...
 static void cx18_writel_expect___2(struct cx18 *cx , u32 val , void *addr ,
                                            u32 eval , u32 mask ) 
{ int i ;
  u32 r ;
 ...
 static void cx18_memcpy_fromio(struct cx18 *cx , void *to , void const   *from ,
                                        unsigned int len ) 
{ 
 ...
 static void cx18_write_reg_expect___2(struct cx18 *cx , u32 val , u32 reg ,
                                               u32 eval , u32 mask ) 
{ 
...
{ u32 str_offset ;
 ...
 static int epu_cmd_irq(struct cx18 *cx , struct cx18_in_work_order *order ) 
{ int ret ;
 ...
 static struct cx18_in_work_order *alloc_in_work_order_irq(struct cx18 *cx ) 
{ int i ;
  struct cx18_in_work_order *order ;
  int tmp ;
...
{ u32 line_size ;
  ...
  if ((u32 )i == lines - 1U) {
    ...
    q += (unsigned long )((line_size - hdr_size) - 4U);
    p += (unsigned long )((line_size - hdr_size) - 1U);
...
static u32 compress_sliced_buf(struct cx18 *cx , u8 *buf , u32 size , u32 hdr_size ) 
{ struct v4l2_decode_vbi_line vbi ;
  ...
  ldv_47458: 
  size = (hdr_size - (unsigned int )i) + size;
  if (size < line_size) {
    return (line);
  }
  i = 0;
...
{ struct cx18_buffer *buf ;
  ...
  mdl->bytesused += buf->bytesused - orig_used;
  __mptr___0 = (struct list_head  const  *)buf->list.next;
  buf = (struct cx18_buffer *)__mptr___0;
  ldv_47501: ;
  if ((unsigned long )(& buf->list) != (unsigned long )(& mdl->buf_list)) {
...
int cx18_av_s_sliced_fmt(struct v4l2_subdev *sd , struct v4l2_sliced_vbi_format *svbi ) ;
 static void cx18_writel___5(struct cx18 *cx , u32 val , void *addr ) 
{ int i ;
  u32 tmp ;
 ...
 static void cx18_writel_expect___5(struct cx18 *cx , u32 val , void *addr ,
                                            u32 eval , u32 mask ) 
{ int i ;
  u32 r ;
...
{ 
 ...
 static void cx18_write_reg___2(struct cx18 *cx , u32 val , u32 reg ) 
{ 
 ...
 static void cx18_write_reg_expect___5(struct cx18 *cx , u32 val , u32 reg ,
                                               u32 eval , u32 mask ) 
{ 
...
{ struct cx18 *cx ;
 ...
 static int cx18_av_dbg_match(struct v4l2_dbg_match  const  *match ) 
{ 
...
{ struct v4l2_subdev *sd ;
  ...
  ldv_47483: 
  dl_control &= 16777215U;
  dl_control |= 251658240U;
...
extern int dvb_net_init(struct dvb_adapter * , struct dvb_net * , struct dmx_demux * ) ;
 static void cx18_writel___7(struct cx18 *cx , u32 val , void *addr ) 
{ int i ;
  u32 tmp ;
 ...
 static void cx18_writel_expect___7(struct cx18 *cx , u32 val , void *addr ,
                                            u32 eval , u32 mask ) 
{ int i ;
  u32 r ;
 ...
 static void cx18_write_reg___3(struct cx18 *cx , u32 val , u32 reg ) 
{ 
 ...
 static void cx18_write_reg_expect___7(struct cx18 *cx , u32 val , u32 reg ,
                                               u32 eval , u32 mask ) 
{ 
...
{ 
 ...
 static u16 cx18_readw(struct cx18 *cx , void const   *addr ) 
{ unsigned short tmp ;
 ...
 static void cx18_writew_noretry(struct cx18 *cx , u16 val , void *addr ) 
{ 
 ...
 static void cx18_writew(struct cx18 *cx , u16 val , void *addr ) 
{ int i ;
  u16 tmp ;
 ...
 static u8 cx18_readb(struct cx18 *cx , void const   *addr ) 
{ unsigned char tmp ;
 ...
 static void cx18_writeb_noretry(struct cx18 *cx , u8 val , void *addr ) 
{ 
 ...
 static void cx18_writeb(struct cx18 *cx , u8 val , void *addr ) 
{ int i ;
  u8 tmp ;
