...
{ struct canfd_frame  const  *cfd ;
  ...
  if ((unsigned int )skb->protocol == 3072U) {
    ...
    if (tmp___1 != 0L) {
      ...
      tmp___2 = ldv__builtin_expect((unsigned int )((unsigned char )cfd->len) > 8U,
                                    0L);
      if (tmp___2 != 0L) {
        goto inval_skb;
      } else
...
int softing_netdev_rx(struct net_device *netdev , struct can_frame  const  *msg ,
                      ktime_t ktime ) ;
 static int canif_is_active(struct net_device *netdev ) 
{ struct can_priv *can ;
  void *tmp ;
  bool tmp___0 ;
  int tmp___1 ;
 ...
 static void softing_set_reset_dpram(struct softing *card ) 
{ unsigned int tmp ;
 ...
 static void softing_clr_reset_dpram(struct softing *card ) 
{ unsigned int tmp ;
...
{ struct net_device *netdev ;
  ...
  if (((int )cmd & 16) != 0) {
    ...
    iowrite8((int )state, (void *)(card->dpram + tmp___10));
...
{ int ret ;
  char const   *tmp ;
  irqreturn_t (*tmp___0)(int irq , void *dev_id ) ;
  ...
  if (card->irq.requested == 0 && enable != 0) {
    ...
    if ((int )(card->pdat)->generation > 1) {
      tmp___0 = & softing_irq_v2;
    } else {
      tmp___0 = & softing_irq_v1;
    }
    ret = request_threaded_irq((unsigned int )card->irq.nr, tmp___0, & softing_irq_thread,
                               128UL, tmp, (void *)card);
    if (ret != 0) {
...
static struct can_bittiming_const  const  softing_btr_const  = 
     {{'s', 'o', 'f', 't', 'i', 'n', 'g', '\000', (char)0, (char)0, (char)0, (char)0,
     (char)0, (char)0, (char)0, (char)0}, 1U, 16U, 1U, 8U, 4U, 1U, 32U, 1U};
...
{ struct net_device *var_group1 ;
  ...
  if (ldv_s_softing_netdev_ops_net_device_ops == 0) {
    ...
    ldv_s_softing_netdev_ops_net_device_ops ++;
  }
  goto ldv_39098;
  case 1: ;
  ...
  if (ldv_s_softing_driver_platform_driver == 0) {
    ...
    ldv_s_softing_driver_platform_driver ++;
  }
  goto ldv_39098;
  case 4: ;
...
extern unsigned long volatile   jiffies ;
 static ktime_t ktime_add_us(ktime_t kt , u64 usec ) 
{ ktime_t __constr_expr_0 ;

  {
  __constr_expr_0.tv64 = (long long )((unsigned long long )kt.tv64 + usec * 1000ULL);
  return (__constr_expr_0);
}
}
...
{ struct firmware  const  *fw ;
  ...
  if (tmp != 0L) {
    ...
    __dynamic_dev_dbg(& descriptor, (struct device  const  *)(& (card->pdev)->dev),
                      ...
                      file, (unsigned int )fw->size, tmp___4, (unsigned int )ret___0);
  }
...
{ uint64_t ovf ;
  ...
  ovf = 68719476736ULL;
  if ((unsigned int )(card->pdat)->freq != 0U) {
    __base = (card->pdat)->freq;
  } else {
    __base = 16U;
  }
...
{ uint64_t rawl ;
 ...
 static int softing_error_reporting(struct net_device *netdev ) 
{ struct softing_priv *priv ;
  void *tmp ;
