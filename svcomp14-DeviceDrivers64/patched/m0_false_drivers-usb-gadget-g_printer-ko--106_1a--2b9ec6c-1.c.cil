...
{ 
 ...
 static int usb_ep_enable(struct usb_ep *ep , struct usb_endpoint_descriptor  const  *desc ) 
{ int tmp ;
 ...
 static int usb_ep_disable(struct usb_ep *ep ) 
{ int tmp ;
 ...
 static struct usb_request *usb_ep_alloc_request(struct usb_ep *ep , gfp_t gfp_flags ) 
{ struct usb_request *tmp ;
 ...
 static void usb_ep_free_request(struct usb_ep *ep , struct usb_request *req ) 
{ 
 ...
 static int usb_ep_queue(struct usb_ep *ep , struct usb_request *req , gfp_t gfp_flags ) 
{ int tmp ;
 ...
 static void set_gadget_data(struct usb_gadget *gadget , void *data ) 
{ 
 ...
 static void *get_gadget_data(struct usb_gadget *gadget ) 
{ void *tmp ;
 ...
 static int usb_gadget_set_selfpowered(struct usb_gadget *gadget ) 
{ int tmp ;
 ...
 static int usb_gadget_vbus_draw(struct usb_gadget *gadget , unsigned int mA ) 
{ int tmp ;
...
{ struct usb_descriptor_header **tmp ;
  ...
  mem += (unsigned long )(n_desc + 1U) * 8UL;
  goto ldv_22371;
  ldv_22370: 
...
static struct usb_descriptor_header  const  *fs_printer_function[11U]  = 
  {      (struct usb_descriptor_header  const  *)(& otg_desc),      (struct usb_descriptor_header  const  *)(& intf_desc),      (struct usb_descriptor_header  const  *)(& fs_ep_in_desc),      (struct usb_descriptor_header  const  *)(& fs_ep_out_desc), 
        0,      0,      0,      0, 
        0,      0,      0};
...
static struct usb_descriptor_header  const  *hs_printer_function[11U]  = 
  {      (struct usb_descriptor_header  const  *)(& otg_desc),      (struct usb_descriptor_header  const  *)(& intf_desc),      (struct usb_descriptor_header  const  *)(& hs_ep_in_desc),      (struct usb_descriptor_header  const  *)(& hs_ep_out_desc), 
        0,      0,      0,      0, 
        0,      0,      0};
...
{ int result ;
  ...
  struct usb_endpoint_descriptor *tmp___0 ;

  {
  result = 0;
  if ((unsigned int )(dev->gadget)->speed == 3U) {
    tmp = & hs_ep_in_desc;
  } else {
    tmp = & fs_ep_in_desc;
  }
  dev->in = (struct usb_endpoint_descriptor  const  *)tmp;
  (dev->in_ep)->driver_data = (void *)dev;
  if ((unsigned int )(dev->gadget)->speed == 3U) {
    tmp___0 = & hs_ep_out_desc;
  } else {
    tmp___0 = & fs_ep_out_desc;
  }
  dev->out = (struct usb_endpoint_descriptor  const  *)tmp___0;
...
{ int result ;
  ...
  ldv_22784: ;
  if (result != 0) {
    if ((unsigned int )*((unsigned char *)dev->gadget + 36UL) != 0U) {
      tmp = 8U;
    } else {
      tmp = 100U;
    }
    usb_gadget_vbus_draw(dev->gadget, tmp);
  } else {
...
extern void *ldv_undef_ptr(void) ;
 static int ldv_nonpositive_int(void) 
{ int ret ;
  int tmp ;
