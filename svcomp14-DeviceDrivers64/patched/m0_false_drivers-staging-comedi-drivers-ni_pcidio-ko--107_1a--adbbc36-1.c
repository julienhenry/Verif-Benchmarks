...
extern int comedi_buf_put(struct comedi_async * , short  ) ;
 static void *comedi_aux_data(int *options , int n ) 
...
extern int mite_buf_change(struct mite_dma_descriptor_ring * , struct comedi_async * ) ;
 static int CHAN_OFFSET(int channel ) 
{ 
 ...
 static int MITE_CHOR(int channel ) 
{ int tmp ;
...
extern void subdev_8255_cleanup(struct comedi_device * , struct comedi_subdevice * ) ;
 static unsigned int primary_DMAChannel_bits(unsigned int channel ) 
{ 
 ...
 static unsigned int secondary_DMAChannel_bits(unsigned int channel ) 
{ 
...
{ 
  ...
  switch (*data) {
  case 1U: 
  s->io_bits |= 1 << ((int )insn->chanspec & 65535);
  goto ldv_24499;
  case 0U: 
  s->io_bits &= ~ (1 << ((int )insn->chanspec & 65535));
  goto ldv_24499;
