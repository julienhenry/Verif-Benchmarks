...
{ 
 ...
 static struct usb_hcd *ohci_to_hcd(struct ohci_hcd  const  *ohci ) 
{ unsigned long const   (*__mptr)[0U] ;
 ...
 static unsigned int _ohci_readl(struct ohci_hcd  const  *ohci , __hc32 *regs ) 
{ unsigned int tmp ;
 ...
 static void _ohci_writel(struct ohci_hcd  const  *ohci , unsigned int val ,
                                  __hc32 *regs ) 
{ 
 ...
 static __hc16 cpu_to_hc16(struct ohci_hcd  const  *ohci , u16 x ) 
{ 
 ...
 static __hc32 cpu_to_hc32(struct ohci_hcd  const  *ohci , u32 x ) 
{ 
 ...
 static u16 hc16_to_cpup(struct ohci_hcd  const  *ohci , __hc16 const   *x ) 
{ __u16 tmp ;
 ...
 static u32 hc32_to_cpu(struct ohci_hcd  const  *ohci , __hc32 x ) 
{ 
 ...
 static u32 hc32_to_cpup(struct ohci_hcd  const  *ohci , __hc32 const   *x ) 
{ __u32 tmp ;
 ...
 static u16 ohci_frame_no(struct ohci_hcd  const  *ohci ) 
{ u32 tmp ;
 ...
 static __hc16 *ohci_hwPSWp(struct ohci_hcd  const  *ohci , struct td  const  *td ,
                                    int index ) 
{ 
 ...
 static u16 ohci_hwPSW(struct ohci_hcd  const  *ohci , struct td  const  *td ,
                               int index ) 
{ __hc16 *tmp ;
  u16 tmp___0 ;
...
{ int status ;
  ...
  if ((unsigned int )*((unsigned char *)tmp___11->self.root_hub + 1672UL) != 0U || autostop != 0) {
    ohci->hc_control |= 1024U;
  } else {
    _ohci_writel((struct ohci_hcd  const  *)ohci, 72U, & (ohci->regs)->intrdisable);
    ohci->hc_control &= 4294966271U;
  }
  ohci->hc_control &= 4294967103U;
  ohci->hc_control |= 192U;
  _ohci_writel((struct ohci_hcd  const  *)ohci, ohci->hc_control, & (ohci->regs)->control);
  _ohci_readl((struct ohci_hcd  const  *)ohci, & (ohci->regs)->control);
  if (autostop == 0) {
 ...
 static struct ed *find_head(struct ed *ed ) 
{ 
...
{ struct ohci_hcd *ohci ;
 ...
 static int root_port_reset(struct ohci_hcd *ohci , unsigned int port ) 
{ __hc32 *portstat ;
  u32 temp ;
  u16 now ;
  unsigned int tmp ;
...
{ struct debug_buffer *tmp ;
 ...
 static void create_debug_files(struct ohci_hcd *ohci ) 
{ struct usb_bus *bus ;
  struct usb_hcd *tmp ;
  struct _ddebug descriptor ;
  struct usb_hcd *tmp___0 ;
 ...
 static void remove_debug_files(struct ohci_hcd *ohci ) 
{ 
...
{ 
 ...
 static struct td *dma_to_td(struct ohci_hcd *hc , dma_addr_t td_dma ) 
{ struct td *td ;

  {
  td_dma &= 4294967264ULL;
...
{ long tmp ;
  ...
  case 0: 
  tmp___0 = ohci_to_hcd((struct ohci_hcd  const  *)ohci);
  (tmp___0->self.bandwidth_isoc_reqs) --;
  tmp___3 = ohci_to_hcd((struct ohci_hcd  const  *)ohci);
...
{ unsigned int i ;
  ...
  ldv_31490: 
  tmp = ohci_to_hcd((struct ohci_hcd  const  *)ohci);
  tmp->self.bandwidth_allocated += (int )ed->load / (int )ed->interval;
  return;
}
}
...
{ struct urb_priv *urb_priv ;
  ...
  (tmp___11->self.bandwidth_isoc_reqs) ++;
  if (tmp___12 == 0) {
    tmp___13 = ohci_to_hcd((struct ohci_hcd  const  *)ohci);
  ...
  if ((tdINFO & 65536U) != 0U) {
    ...
    if ((type != 2 || (unsigned int )td->index != 0U) && tdBE != 0U) {
      if (td->hwCBP == 0U) {
        ...
        urb->actual_length += tmp___2 - (u32 )td->data_dma;
      }
    }
  }
  return (status);
...
{ u32 mask ;
  ...
  if (first != 0) {
    ...
    ohci->fminterval |= ((ohci->fminterval * 6U + 4294966036U) / 7U & 32767U) << 16;
  }
