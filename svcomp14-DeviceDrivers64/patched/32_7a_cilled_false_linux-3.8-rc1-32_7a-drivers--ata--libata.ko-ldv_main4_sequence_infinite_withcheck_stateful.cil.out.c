...
extern void scsi_host_put(struct Scsi_Host * ) ;
 static unsigned int ata_prot_flags(u8 prot ) 
{ 
 ...
 static int ata_is_nodata(u8 prot ) 
{ unsigned int tmp ;
...
{ unsigned int tmp ;
 ...
 static int ata_is_data(u8 prot ) 
{ unsigned int tmp ;
 ...
 static bool ata_id_has_flush_ext(u16 const   *id ) 
{ 
 ...
 static bool ata_id_has_lba48(u16 const   *id ) 
{ 
 ...
 static bool ata_id_hpa_enabled(u16 const   *id ) 
{ 
...
{ unsigned int mver ;
 ...
 static bool ata_id_is_sata(u16 const   *id ) 
{ 
 ...
 static bool ata_id_has_tpm(u16 const   *id ) 
{ unsigned int tmp ;
 ...
 static bool ata_id_current_chs_valid(u16 const   *id ) 
{ 
 ...
 static bool ata_id_is_cfa(u16 const   *id ) 
{ 
 ...
 static bool ata_drive_40wire(u16 const   *dev_id ) 
{ bool tmp ;
 ...
 static bool ata_drive_40wire_relaxed(u16 const   *dev_id ) 
{ 
 ...
 static int atapi_cdb_len(u16 const   *dev_id ) 
{ u16 tmp ;
 ...
 static bool atapi_command_packet_set(u16 const   *dev_id ) 
{ 
 ...
 static bool atapi_id_dmadir(u16 const   *dev_id ) 
{ unsigned int tmp ;
 ...
 static bool lba_28_ok(u64 block , u32 n_block ) 
{ 
 ...
 static bool lba_48_ok(u64 block , u32 n_block ) 
{ 
...
struct ata_port_info  const  ata_dummy_port_info ;
 static unsigned long const   *sata_ehc_deb_timing(struct ata_eh_context *ehc ) 
{ 
 ...
 static int ata_port_is_dummy(struct ata_port *ap ) 
{ 
...
struct ata_port_operations  const  sata_port_ops ;
 static bool sata_pmp_supported(struct ata_port *ap ) 
{ 
 ...
 static bool sata_pmp_attached(struct ata_port *ap ) 
{ 
 ...
 static int ata_is_host_link(struct ata_link  const  *link ) 
{ 
...
void ata_port_desc(struct ata_port *ap , char const   *fmt  , ...) ;
 static unsigned int ata_tag_valid(unsigned int tag ) 
{ 
...
{ 
 ...
 static unsigned int ata_dev_enabled(struct ata_device  const  *dev ) 
{ unsigned int tmp ;
 ...
 static int ata_link_max_devices(struct ata_link  const  *link ) 
{ int tmp ;
...
struct ata_device *ata_dev_next(struct ata_device *dev , struct ata_link *link , enum ata_dev_iter_mode mode ) ;
 static int ata_ncq_enabled(struct ata_device *dev ) 
{ 
 ...
 static struct ata_queued_cmd *__ata_qc_from_tag(struct ata_port *ap , unsigned int tag ) 
{ unsigned int tmp ;
  long tmp___0 ;
 ...
 static struct ata_queued_cmd *ata_qc_from_tag(struct ata_port *ap , unsigned int tag ) 
{ struct ata_queued_cmd *qc ;
 ...
 static void ata_tf_init(struct ata_device *dev , struct ata_taskfile *tf ) 
{ 
...
{ unsigned int tmp ;
 ...
 static unsigned long ata_deadline(unsigned long from_jiffies , unsigned long timeout_msecs ) 
{ unsigned long tmp ;
 ...
 static int ata_dma_enabled(struct ata_device *adev ) 
{ 
...
extern void __pm_runtime_disable(struct device * , bool  ) ;
 static bool pm_runtime_suspended(struct device *dev ) 
{ 
...
{ bool did_spd ;
  ...
  if ((unsigned int )fe->param.lflags != 0U) {
    link->flags |= (unsigned int )fe->param.lflags;
...
{ int devno ;
  ...
  dev->horkage |= (unsigned int )fe->param.horkage_on;
  dev->horkage &= (unsigned int )(~ fe->param.horkage_off);
...
{ bool tmp ;
  ...
  {
  tf->flags |= 6UL;
  tf->flags |= (unsigned long )tf_flags;
...
{ struct ata_eh_context *ehc ;
 ...
 static void ata_dump_id(u16 const   *id ) 
{ 
...
{ struct ata_link *plink ;
 ...
 static u8 ata_dev_knobble(struct ata_device *dev ) 
{ struct ata_port *ap ;
  unsigned long tmp ;
  bool tmp___0 ;
  int tmp___1 ;
...
{ struct ata_port *ap ;
  ...
  dev->horkage |= (unsigned int )tmp___0;
  ...
  if ((atapi_enabled == 0 || (ap->flags & 64UL) != 0UL) && dev->class == 3U) {
    if (atapi_enabled != 0) {
      tmp___18 = (char *)"not supported with this driver";
    } else {
      tmp___18 = (char *)"disabled";
    }
  ...
  dev->flags &= 0xfffffffffffff000UL;
  dev->max_sectors = 0U;
  dev->cdb_len = 0U;
  dev->n_sectors = 0ULL;
  dev->cylinders = 0U;
  ...
  if ((dev->horkage & 8U) != 0U) {
    ...
    if (__min1 < __min2) {
      dev->max_sectors = __min1;
    } else {
      dev->max_sectors = __min2;
    }
  }
...
{ unsigned int classes[2U] ;
  ...
  goto ldv_40627;
  ldv_40626: 
  ap->link.eh_context.i.flags |= 262144U;
  rc = ata_dev_configure(dev);
  ap->link.eh_context.i.flags &= 4294705151U;
  if (rc != 0) {
    goto fail;
  }
  dev = ata_dev_next(dev, & ap->link, 0);
  ...
  _min2 = 1;
  if (_min1 < _min2) {
    tries[dev->devno] = _min1;
  } else {
    tries[dev->devno] = _min2;
  }
  case -5: ;
...
{ 
  ...
  {
  if ((int )t->setup * 1000 != 0) {
    q->setup = (unsigned int )((unsigned short )(((int )t->setup * 1000 + -1) / T)) + 1U;
  } else {
    q->setup = 0U;
  }
  if ((int )t->act8b * 1000 != 0) {
    q->act8b = (unsigned int )((unsigned short )(((int )t->act8b * 1000 + -1) / T)) + 1U;
  } else {
    q->act8b = 0U;
  }
  if ((int )t->rec8b * 1000 != 0) {
    q->rec8b = (unsigned int )((unsigned short )(((int )t->rec8b * 1000 + -1) / T)) + 1U;
  } else {
    q->rec8b = 0U;
  }
  if ((int )t->cyc8b * 1000 != 0) {
    q->cyc8b = (unsigned int )((unsigned short )(((int )t->cyc8b * 1000 + -1) / T)) + 1U;
  } else {
    q->cyc8b = 0U;
  }
  if ((int )t->active * 1000 != 0) {
    q->active = (unsigned int )((unsigned short )(((int )t->active * 1000 + -1) / T)) + 1U;
  } else {
    q->active = 0U;
  }
  if ((int )t->recover * 1000 != 0) {
    q->recover = (unsigned int )((unsigned short )(((int )t->recover * 1000 + -1) / T)) + 1U;
  } else {
    q->recover = 0U;
  }
  if ((int )t->dmack_hold * 1000 != 0) {
    q->dmack_hold = (unsigned int )((unsigned short )(((int )t->dmack_hold * 1000 + -1) / T)) + 1U;
  } else {
    q->dmack_hold = 0U;
  }
  if ((int )t->cycle * 1000 != 0) {
    q->cycle = (unsigned int )((unsigned short )(((int )t->cycle * 1000 + -1) / T)) + 1U;
  } else {
    q->cycle = 0U;
  }
  if ((int )t->udma * 1000 != 0) {
    q->udma = (unsigned int )((unsigned short )(((int )t->udma * 1000 + -1) / UT)) + 1U;
  } else {
    q->udma = 0U;
  }
  return;
...
{ unsigned short _max1 ;
  ...
  if ((what & 2U) != 0U) {
    ...
    m->act8b = (unsigned short )tmp___0;
  }
  if ((what & 4U) != 0U) {
    _max1___1 = a->rec8b;
    _max2___1 = b->rec8b;
    if ((int )_max1___1 > (int )_max2___1) {
      tmp___1 = _max1___1;
    } else {
      tmp___1 = _max2___1;
    }
    m->rec8b = (unsigned short )tmp___1;
  }
  if ((what & 8U) != 0U) {
    _max1___2 = a->cyc8b;
    _max2___2 = b->cyc8b;
    if ((int )_max1___2 > (int )_max2___2) {
      tmp___2 = _max1___2;
    } else {
      tmp___2 = _max2___2;
    }
    m->cyc8b = (unsigned short )tmp___2;
  }
  if ((what & 16U) != 0U) {
    _max1___3 = a->active;
    _max2___3 = b->active;
    if ((int )_max1___3 > (int )_max2___3) {
      tmp___3 = _max1___3;
    } else {
      tmp___3 = _max2___3;
    }
    m->active = (unsigned short )tmp___3;
  }
  if ((what & 32U) != 0U) {
    _max1___4 = a->recover;
    _max2___4 = b->recover;
    if ((int )_max1___4 > (int )_max2___4) {
      tmp___4 = _max1___4;
    } else {
      tmp___4 = _max2___4;
    }
    m->recover = (unsigned short )tmp___4;
  }
  if ((what & 64U) != 0U) {
    _max1___5 = a->dmack_hold;
    _max2___5 = b->dmack_hold;
    if ((int )_max1___5 > (int )_max2___5) {
      tmp___5 = _max1___5;
    } else {
      tmp___5 = _max2___5;
    }
    m->dmack_hold = (unsigned short )tmp___5;
  }
  if ((what & 128U) != 0U) {
    _max1___6 = a->cycle;
    _max2___6 = b->cycle;
    if ((int )_max1___6 > (int )_max2___6) {
      tmp___6 = _max1___6;
    } else {
      tmp___6 = _max2___6;
    }
    m->cycle = (unsigned short )tmp___6;
  }
  if ((what & 256U) != 0U) {
    _max1___7 = a->udma;
    _max2___7 = b->udma;
    if ((int )_max1___7 > (int )_max2___7) {
      tmp___7 = _max1___7;
    } else {
      tmp___7 = _max2___7;
    }
    m->udma = (unsigned short )tmp___7;
  }
  return;
}
}
...
{ struct ata_port *ap ;
  ...
  ign_dev_err = 0;
  err_mask = 0U;
  dev->flags &= 0xffffffffffffefffUL;
  ...
  ehc->i.flags |= 1048576U;
  rc = ata_dev_revalidate(dev, 0U, 0U);
  ehc->i.flags &= 4293918719U;
  if (rc != 0) {
    return (rc);
  }
  if (dev->xfer_shift == 0U) {
...
{ struct ata_eh_context *ehc ;
  ...
  goto ldv_40923;
  case 2U: 
  scontrol &= 4294967039U;
  scontrol |= 1536U;
  goto ldv_40923;
  case 3U: 
  tmp = ata_link_nr_enabled(link);
  if (tmp > 0) {
    scontrol &= 4294965503U;
  } else {
    scontrol &= 4294967280U;
    scontrol |= 4U;
  }
  goto ldv_40923;
  default: 
...
{ struct ata_link *link ;
  ...
  if ((unsigned long )tmp___1 != (unsigned long )((struct ata_device *)0)) {
    xfer_mask &= 0xffffffffffffff9fUL;
    xfer_mask &= 0xfffffffffffff3ffUL;
  }
  tmp___2 = ata_dma_blacklisted((struct ata_device  const  *)dev);
  if (tmp___2 != 0) {
    xfer_mask &= 0xfffffffffff0007fUL;
  ...
  if ((xfer_mask & 1015808UL) != 0UL) {
    ...
    if (tmp___4 != 0) {
      ...
      xfer_mask &= 0xfffffffffff07fffUL;
    }
  }
  ata_unpack_xfermask(xfer_mask, & dev->pio_mask, & dev->mwdma_mask, & dev->udma_mask);
  return;
...
{ struct ata_port *ap ;
  ...
  qc->flags &= 0xfffffffffffffffeUL;
  ap->qc_active &= (unsigned int )(~ (1 << (int )qc->tag));
  (*(qc->complete_fn))(qc);
  return;
}
}
...
{ struct ata_device *dev ;
  ...
  dev->flags &= 0xfffffffffffeffffUL;
  return;
}
}
void ata_qc_complete(struct ata_queued_cmd *qc ) 
...
{ struct ata_link *link ;
  ...
  tmp___0 = spinlock_check(ap->lock);
  flags = _raw_spin_lock_irqsave(tmp___0);
  dev->flags &= 0xffffffffff000000UL;
  dev->horkage = 0U;
  spin_unlock_irqrestore(ap->lock, flags);
...
{ u8 spd ;
  ...
  if ((unsigned int )spd != 0U) {
    link->hw_sata_spd_limit &= (unsigned int )((1 << (int )spd) + -1);
  }
...
{ struct ata_port_info  const  *pi ;
  ...
  ap->flags |= (unsigned long )pi->flags;
  ap->link.flags |= (unsigned int )pi->link_flags;
  ap->ops = pi->port_ops;
...
extern int scsi_execute(struct scsi_device * , unsigned char const   * , int  , void * ,
 ...
 static bool ata_id_has_fua(u16 const   *id ) 
{ 
 ...
 static bool ata_id_has_flush(u16 const   *id ) 
{ 
 ...
 static u32 ata_id_logical_sector_size(u16 const   *id ) 
{ 
 ...
 static u8 ata_id_log2_per_physical_sector(u16 const   *id ) 
{ 
 ...
 static u16 ata_id_logical_sector_offset(u16 const   *id , u8 log2_per_phys ) 
{ u16 word_209 ;
  u16 first ;
 ...
 static bool ata_id_rahead_enabled(u16 const   *id ) 
{ 
 ...
 static bool ata_id_wcache_enabled(u16 const   *id ) 
{ 
 ...
 static unsigned int ata_id_major_version___0(u16 const   *id ) 
{ unsigned int mver ;
 ...
 static bool ata_id_has_unload(u16 const   *id ) 
{ unsigned int tmp ;
 ...
 static bool ata_id_has_wwn(u16 const   *id ) 
{ 
 ...
 static int ata_id_form_factor(u16 const   *id ) 
{ u16 val ;
  unsigned int tmp ;
 ...
 static int ata_id_rotation_rate(u16 const   *id ) 
{ u16 val ;
  unsigned int tmp ;
 ...
 static bool ata_id_has_trim(u16 const   *id ) 
{ unsigned int tmp ;
 ...
 static bool ata_id_has_zero_after_trim(u16 const   *id ) 
{ bool tmp ;
 ...
 static unsigned int ata_set_lba_range_entries(void *_buffer , unsigned int buf_size ,
...
{ __le64 *buffer ;
  ...
  entry = ((unsigned long long )tmp___0 << 48) | sector;
  tmp = i;
  i ++;
...
struct device_attribute *ata_common_sdev_attrs[2U] ;
 static unsigned int ata_qc_raw_nbytes(struct ata_queued_cmd *qc ) 
{ unsigned int _min1 ;
  unsigned int _min2 ;
  unsigned int tmp ;
  ...
  return (qc->nbytes - tmp);
}
}
 static int ata_try_flush_cache(struct ata_device  const  *dev ) 
{ bool tmp ;
  bool tmp___0 ;
  bool tmp___1 ;
  int tmp___2 ;
 ...
 static struct ata_port *ata_shost_to_port(struct Scsi_Host *host ) 
{ 
...
{ struct scsi_device *sdev ;
  ...
  if (input >= 0L) {
    ...
    (dev->link)->eh_info.dev_action[dev->devno] |= 32U;
    ata_port_schedule_eh(ap);
    complete(& ap->park_req_pending);
  } else {
    switch (input) {
    case -1L: 
    dev->flags &= 0xfffffffffffdffffUL;
    goto ldv_38329;
    case -2L: 
    dev->flags |= 131072UL;
    goto ldv_38329;
...
{ struct ata_device *dev ;
  ...
  tmp___0 = spinlock_check(ap->lock);
  flags = _raw_spin_lock_irqsave(tmp___0);
  dev->flags &= 0xffffffffffffdfffUL;
  if (queue_depth == 1) {
    dev->flags |= 8192UL;
    queue_depth = 1;
  } else {
  ...
  if (_min1___0 < _min2___0) {
    queue_depth = _min1___0;
  } else {
    queue_depth = _min2___0;
  }
  _min1___1 = queue_depth;
  _min2___1 = 31;
  if (_min1___1 < _min2___1) {
    queue_depth = _min1___1;
  } else {
    queue_depth = _min2___1;
  }
...
{ raw_spinlock_t *tmp ;
 ...
 static void ata_scsi_rbuf_put(struct scsi_cmnd *cmd , bool copy_out , unsigned long *flags ) 
{ unsigned int tmp ;
...
{ 
 ...
 static int ata_pio_use_silly(struct ata_port *ap ) 
{ 
...
{ struct ata_taskfile *tf ;
  ...
  if (dev->devno != 0U) {
    tf->device = (u8 )((unsigned int )tf->device | 16U);
  } else {
    tf->device = (unsigned int )tf->device & 239U;
  }
...
{ int tmp ;
 ...
 static void ata_scsi_dump_cdb(struct ata_port *ap , struct scsi_cmnd *cmd ) 
{ 
 ...
 static int __ata_scsi_queuecmd(struct scsi_cmnd *scmd , struct ata_device *dev ) 
{ u8 scsi_op ;
...
{ struct ata_port *ap ;
  ...
  tmp = spinlock_check(ap->lock);
  flags = _raw_spin_lock_irqsave(tmp);
  dev->flags &= 0xfffffffffdffffffUL;
  spin_unlock_irqrestore(ap->lock, flags);
...
{ struct ata_port *ap ;
  ...
  if (devno == -1) {
    ...
    if ((unsigned long )dev != (unsigned long )((struct ata_device *)0)) {
      ...
      ehi___0->probe_mask |= (unsigned int )(1 << (int )dev->devno);
      ehi___0->action |= 6U;
    } else {
      rc = -22;
    }
...
extern struct pv_lock_ops pv_lock_ops ;
 static int arch_spin_is_locked(struct arch_spinlock *lock ) 
{ int __ret ;
  unsigned long __edi ;
  unsigned long __esi ;
  unsigned long __edx ;
...
void ldv_mutex_unlock_eh_mutex(struct mutex *lock ) ;
 static void arch_spin_unlock_wait(arch_spinlock_t *lock ) 
{ int tmp ;
...
{ 
 ...
 static u64 get_jiffies_64(void) 
{ 
...
extern void scsi_schedule_eh(struct Scsi_Host * ) ;
 static int ata_is_atapi(u8 prot ) 
{ unsigned int tmp ;
 ...
 static bool ata_id_has_hipm(u16 const   *id ) 
{ u16 val ;
 ...
 static bool ata_id_has_dipm(u16 const   *id ) 
{ u16 val ;
...
void __ata_ehi_push_desc(struct ata_eh_info *ehi , char const   *fmt  , ...) ;
 static void ata_ehi_hotplugged(struct ata_eh_info *ehi ) 
{ 
...
{ struct pci_dev *pdev ;
  ...
  if (pdev->resource[bar].start != 0ULL || pdev->resource[bar].end != pdev->resource[bar].start) {
    len = (pdev->resource[bar].end - pdev->resource[bar].start) + 1ULL;
  } else {
    len = 0ULL;
  }
  if (offset < 0L) {
...
{ struct ata_device *tdev ;
  ...
  if ((unsigned long )dev == (unsigned long )((struct ata_device *)0)) {
    ...
    if ((ehi->action & action) != 0U) {
      ...
      goto ldv_38132;
      ldv_38131: 
      ehi->dev_action[tdev->devno] |= ehi->action & action;
      tdev = ata_dev_next(tdev, link, 2);
      ldv_38132: ;
      if ((unsigned long )tdev != (unsigned long )((struct ata_device *)0)) {
        goto ldv_38131;
...
{ unsigned long flags ;
  ...
  if ((unsigned long )(ap->ops)->error_handler != (unsigned long )((void (*)(struct ata_port * ))0)) {
    ...
    if (tmp___0 != 0) {
      ehc->saved_ncq_enabled |= (unsigned int )(1 << devno);
    }
    dev = ata_dev_next(dev, link, 0);
    ldv_38238: ;
    if ((unsigned long )dev != (unsigned long )((struct ata_device *)0)) {
  ...
  ap->pflags &= 4294967223U;
  ap->pflags &= 4294967293U;
...
{ struct ata_link *link ;
  ...
  tmp = spinlock_check(ap->lock);
  flags = _raw_spin_lock_irqsave(tmp);
  dev->flags &= 0xfffffffffeffffffUL;
  tmp___0 = ata_scsi_offline_dev(dev);
  if (tmp___0 != 0) {
    dev->flags |= 33554432UL;
    ap->pflags |= 64U;
  }
  ...
  ehc->saved_ncq_enabled &= (unsigned int )(~ (1 << (int )dev->devno));
  spin_unlock_irqrestore(ap->lock, flags);
  return;
}
}
...
{ u64 j5mins ;
  ...
  _min1 = j64;
  _min2 = j5mins;
  if ((unsigned long long const   )_min1 < (unsigned long long const   )_min2) {
    tmp___0 = (unsigned long long const   )_min1;
  } else {
    tmp___0 = _min2;
  }
  arg.since = j64 - (unsigned long long )tmp___0;
  ...
  _min1___0 = j64;
  _min2___0 = j10mins;
  if ((unsigned long long const   )_min1___0 < (unsigned long long const   )_min2___0) {
    tmp___1 = (unsigned long long const   )_min1___0;
  } else {
    tmp___1 = _min2___0;
  }
  arg.since = j64 - (unsigned long long )tmp___1;
  ata_ering_map(& dev->ering, & speed_down_verdict_cb, (void *)(& arg));
...
{ struct ata_link *link ;
 ...
 static int ata_eh_worth_retry(struct ata_queued_cmd *qc ) 
{ 
...
{ struct ata_port *ap ;
  ...
  ehc->i.action |= tmp___1;
  if ((qc->err_mask & 16U) != 0U) {
    qc->err_mask &= 4294967158U;
  }
  if ((qc->err_mask & 4294967039U) != 0U) {
    qc->err_mask &= 4294967039U;
  }
  if ((qc->flags & 131072UL) != 0UL) {
    qc->err_mask &= 4294967038U;
  }
  ...
  if ((unsigned long )ehc->i.dev != (unsigned long )((struct ata_device *)0)) {
    ehc->i.dev_action[(ehc->i.dev)->devno] |= ehc->i.action & 33U;
    ehc->i.action &= 4294967262U;
  }
  if ((all_err_mask & 4U) != 0U) {
    tmp___3 = ata_is_host_link((struct ata_link  const  *)link);
    if (tmp___3 == 0) {
      ap->link.eh_context.i.err_mask |= 4U;
    }
  }
  dev = ehc->i.dev;
  if ((unsigned long )dev == (unsigned long )((struct ata_device *)0)) {
...
{ struct ata_port *ap ;
  ...
  if (ehc->i.serror != 0U) {
    if ((ehc->i.serror & 67108864U) != 0U) {
      tmp___8 = (char *)"DevExch ";
    } else {
      tmp___8 = (char *)"";
    }
    if ((ehc->i.serror & 33554432U) != 0U) {
      tmp___9 = (char *)"UnrecFIS ";
    } else {
      tmp___9 = (char *)"";
    }
    if ((ehc->i.serror & 16777216U) != 0U) {
      tmp___10 = (char *)"TrStaTrns ";
    } else {
      tmp___10 = (char *)"";
    }
    if ((ehc->i.serror & 8388608U) != 0U) {
      tmp___11 = (char *)"LinkSeq ";
    } else {
      tmp___11 = (char *)"";
    }
    if ((ehc->i.serror & 4194304U) != 0U) {
      tmp___12 = (char *)"Handshk ";
    } else {
      tmp___12 = (char *)"";
    }
    if ((ehc->i.serror & 2097152U) != 0U) {
      tmp___13 = (char *)"BadCRC ";
    } else {
      tmp___13 = (char *)"";
    }
    if ((ehc->i.serror & 1048576U) != 0U) {
      tmp___14 = (char *)"Dispar ";
    } else {
      tmp___14 = (char *)"";
    }
    if ((ehc->i.serror & 524288U) != 0U) {
      tmp___15 = (char *)"10B8B ";
    } else {
      tmp___15 = (char *)"";
    }
    if ((ehc->i.serror & 262144U) != 0U) {
      tmp___16 = (char *)"CommWake ";
    } else {
      tmp___16 = (char *)"";
    }
    if ((ehc->i.serror & 131072U) != 0U) {
      tmp___17 = (char *)"PHYInt ";
    } else {
      tmp___17 = (char *)"";
    }
    if ((ehc->i.serror & 65536U) != 0U) {
      tmp___18 = (char *)"PHYRdyChg ";
    } else {
      tmp___18 = (char *)"";
    }
    if ((ehc->i.serror & 2048U) != 0U) {
      tmp___19 = (char *)"HostInt ";
    } else {
      tmp___19 = (char *)"";
    }
    if ((ehc->i.serror & 1024U) != 0U) {
      tmp___20 = (char *)"Proto ";
    } else {
      tmp___20 = (char *)"";
    }
    if ((ehc->i.serror & 512U) != 0U) {
      tmp___21 = (char *)"Persist ";
    } else {
      tmp___21 = (char *)"";
    }
    if ((ehc->i.serror & 256U) != 0U) {
      tmp___22 = (char *)"UnrecovData ";
    } else {
      tmp___22 = (char *)"";
    }
    if ((ehc->i.serror & 2U) != 0U) {
      tmp___23 = (char *)"RecovComm ";
    } else {
      tmp___23 = (char *)"";
    }
    if ((int )ehc->i.serror & 1) {
      tmp___24 = (char *)"RecovData ";
    } else {
      tmp___24 = (char *)"";
    }
    ata_link_printk((struct ata_link  const  *)link, "\v", "SError: { %s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s}\n",
                    tmp___24, tmp___23, tmp___22, tmp___21, tmp___20, tmp___19, tmp___18,
                    tmp___17, tmp___16, tmp___15, tmp___14, tmp___13, tmp___12, tmp___11,
  ...
  if (((int )res->command & 233) != 0) {
    if ((int )((signed char )res->command) < 0) {
      ...
      if ((int )res->command & 1) {
        tmp___26 = (char *)"ERR ";
      } else {
        tmp___26 = (char *)"";
      }
      if (((int )res->command & 8) != 0) {
        tmp___27 = (char *)"DRQ ";
      } else {
        tmp___27 = (char *)"";
      }
      if (((int )res->command & 32) != 0) {
        tmp___28 = (char *)"DF ";
      } else {
        tmp___28 = (char *)"";
      }
      if (((int )res->command & 64) != 0) {
        tmp___29 = (char *)"DRDY ";
      } else {
        tmp___29 = (char *)"";
      }
      ata_dev_printk((struct ata_device  const  *)qc___0->dev, "\v", "status: { %s%s%s%s}\n",
                     tmp___29, tmp___28, tmp___27, tmp___26);
    }
  }
  if ((unsigned int )cmd->command != 160U && ((int )res->feature & 212) != 0) {
    if (((int )res->feature & 4) != 0) {
      tmp___30 = (char *)"ABRT ";
    } else {
      tmp___30 = (char *)"";
    }
    if (((int )res->feature & 16) != 0) {
      tmp___31 = (char *)"IDNF ";
    } else {
      tmp___31 = (char *)"";
    }
    if (((int )res->feature & 64) != 0) {
      tmp___32 = (char *)"UNC ";
    } else {
      tmp___32 = (char *)"";
    }
    if ((int )((signed char )res->feature) < 0) {
      tmp___33 = (char *)"ICRC ";
    } else {
      tmp___33 = (char *)"";
    }
    ata_dev_printk((struct ata_device  const  *)qc___0->dev, "\v", "error: { %s%s%s%s}\n",
                   tmp___33, tmp___32, tmp___31, tmp___30);
  }
  ldv_38630: 
  tag ++;
  ldv_38635: ;
...
{ struct ata_port *ap ;
  ...
  if ((unsigned long )reset != (unsigned long )((int (*)(struct ata_link * , unsigned int * ,
                                                         unsigned long  ))0)) {
    if (verbose != 0) {
      if ((unsigned long )reset == (unsigned long )softreset) {
        tmp___26 = (char *)"soft";
      } else {
        tmp___26 = (char *)"hard";
      }
  ...
  ldv_38725: 
  dev->pio_mode = 8U;
  dev->flags &= 0xffffffffffff7fffUL;
  ...
  rc = 0;
  out: 
  ehc->i.flags &= 4294967294U;
  if ((unsigned long )slave != (unsigned long )((struct ata_link *)0)) {
    sehc->i.flags &= 4294967294U;
  }
  tmp___19 = spinlock_check(ap->lock);
  flags = _raw_spin_lock_irqsave(tmp___19);
  ap->pflags &= 4294967039U;
  spin_unlock_irqrestore(ap->lock, flags);
  return (rc);
  fail: 
 ...
 static void ata_eh_pull_park_action(struct ata_port *ap ) 
{ struct ata_link *link ;
  struct ata_device *dev ;
  unsigned long flags ;
  raw_spinlock_t *tmp ;
  ...
  ldv_38757: 
  ehi = & link->eh_info;
  link->eh_context.i.dev_action[dev->devno] |= ehi->dev_action[dev->devno] & 32U;
...
{ struct ata_eh_context *ehc ;
  ...
  ata_tf_init(dev, & tf);
  if (park != 0) {
    ehc->unloaded_mask |= (unsigned int )(1 << (int )dev->devno);
    tf.command = 225U;
    tf.feature = 68U;
    tf.lbal = 76U;
    tf.lbam = 78U;
...
{ struct ata_link *link ;
  ...
  if (err_mask == 0U) {
    ...
    _max2 = 1;
    if (_max1 > _max2) {
      (qc->scsicmd)->allowed = _max1;
    } else {
      (qc->scsicmd)->allowed = _max2;
    }
  } else {
...
{ struct ata_port *ap ;
  ...
  if (rc != 0) {
    ...
    if ((unsigned long )lpm_dev != (unsigned long )((struct ata_device *)0)) {
      dev = lpm_dev;
    } else {
      dev = link_dev;
    }
...
{ struct ata_eh_context *ehc ;
  ...
  ata_dev_init(dev);
  ehc->did_probe_mask |= (unsigned int )(1 << (int )dev->devno);
  ehc->i.action |= 6U;
  ehc->saved_xfer_mode[dev->devno] = 0U;
  ehc->saved_ncq_enabled &= (unsigned int )(~ (1 << (int )dev->devno));
  if ((unsigned int )link->lpm_policy > 1U) {
...
{ struct ata_eh_context *ehc ;
  ...
  switch (err) {
  case -19: 
  ehc->i.probe_mask |= (unsigned int )(1 << (int )dev->devno);
  case -22: 
  _min1 = ehc->tries[dev->devno];
  _min2 = 1;
  if (_min1 < _min2) {
    ehc->tries[dev->devno] = _min1;
  } else {
    ehc->tries[dev->devno] = _min2;
  }
  case -5: ;
...
{ struct ata_link *link ;
  ...
  ehc->i.action |= ehc->i.dev_action[dev->devno] & 4294967262U;
  ehc->i.dev_action[dev->devno] &= 33U;
...
extern void jiffies_to_timespec(unsigned long  , struct timespec * ) ;
...
extern void transport_destroy_device(struct device * ) ;
 static int transport_container_register(struct transport_container *tc ) 
{ int tmp ;
 ...
 static void transport_container_unregister(struct transport_container *tc ) 
{ int tmp ;
  long tmp___0 ;
...
extern void pcim_pin_device(struct pci_dev * ) ;
 static int PageSlab(struct page  const  *page ) 
{ int tmp ;
...
{ 
 ...
 static void ata_qc_set_polling(struct ata_queued_cmd *qc ) 
{ 
...
{ unsigned int mask ;
 ...
 static int ata_check_ready(u8 status ) 
{ 
...
int ata_pci_bmdma_init_one(struct pci_dev *pdev , struct ata_port_info  const  * const  *ppi ,
 ...
 static u8 ata_sff_busy_wait(struct ata_port *ap , unsigned int bits , unsigned int max ) 
{ u8 status ;
...
{ 
 ...
 static void ata_tf_to_host(struct ata_port *ap , struct ata_taskfile  const  *tf ) 
{ 
...
{ struct ata_port *ap ;
  ...
  if (tmp___0 != 0L) {
    ...
    buf += (unsigned long )(buflen - (unsigned int )slop);
    if (rw == 0) {
      if (slop <= 2) {
        ioread16_rep(data_addr, (void *)(& pad), 1UL);
      } else {
...
{ unsigned int tmp ;
 ...
 static irqreturn_t __ata_sff_interrupt(int irq , void *dev_instance , unsigned int (*port_intr)(struct ata_port * ,
                                                                                                         struct ata_queued_cmd * ) ) 
{ struct ata_host *host ;
  bool retried ;
  unsigned int i ;
...
{ struct device *dev ;
  ...
  host->flags |= (unsigned long )hflags;
  if ((int )bmdma) {
    pci_set_master(pdev);
...
{ struct ata_port *ap ;
  ...
  (prd + (unsigned long )(pi - 1U))->flags_len |= 2147483648U;
  return;
}
}
...
void ldv_mutex_lock_100(struct mutex *ldv_func_arg1 ) ;
 static int ata_link_active(struct ata_link *link ) 
{ unsigned int tmp ;
...
static unsigned int sata_pmp_read(struct ata_link *link ,
                                  int reg , u32 *r_val ) 
{ struct ata_port *ap ;
...
{ u32 *gscr ;
  ...
  if ((unsigned int )vendor == 4245U && ((((unsigned int )devid == 22307U || (unsigned int )devid == 22323U) || (unsigned int )devid == 22324U) || (unsigned int )devid == 22340U)) {
    (ap->pmp_link + ((unsigned long )ap->nr_pmp_links + 0xffffffffffffffffUL))->flags |= 32U;
  } else
...
{ struct ata_port *ap ;
  ...
  if (*(link_tries + (unsigned long )link->pmp) != 0) {
    (*(link_tries + (unsigned long )link->pmp)) --;
    if (*(link_tries + (unsigned long )link->pmp) != 0) {
      return (1);
    }
  }
...
extern int scsi_is_host_device(struct device  const  * ) ;
 static struct ata_acpi_gtm  const  *ata_acpi_init_gtm(struct ata_port *ap ) 
{ 
...
extern struct bus_type pci_bus_type ;
 static void device_set_run_wake(struct device *dev , bool enable ) 
{ 
...
{ struct ata_taskfile *pptf ;
  ...
  ata_dev_printk((struct ata_device  const  *)dev, level, "ACPI cmd %02x/%02x:%02x:%02x:%02x:%02x:%02x (%s) %s\n",
                 ...
                 (int )tf.lbam, (int )tf.lbah, (int )tf.device, tmp___0, (char *)(& msg));
  return (rc);
}
}
...
{ struct ata_port *ap ;
  ...
  dev->flags &= 0xffffffffffffffdfUL;
  if (nr_executed != 0) {
    rc = ata_dev_reread_id(dev, 0U);
    if (rc < 0) {
