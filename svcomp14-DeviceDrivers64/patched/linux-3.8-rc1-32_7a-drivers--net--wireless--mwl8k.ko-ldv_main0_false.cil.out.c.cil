...
{ int tmp ;
 ...
 static int ldv_mutex_is_locked_9(struct mutex *lock ) ;
...
{ 
 ...
 static int ieee80211_is_data(__le16 fc ) 
{ 
...
{ 
 ...
 static int ieee80211_is_auth(__le16 fc ) 
{ 
...
{ int tmp ;
 ...
 static enum nl80211_channel_type cfg80211_get_chandef_type(struct cfg80211_chan_def  const  *chandef ) 
{ int __ret_warn_on ;
  long tmp ;
...
{ struct mwl8k_priv *priv ;
 ...
 static void mwl8k_remove_dma_header(struct sk_buff *skb , __le16 qos ) 
...
{ int tmp ;
 ...
 static void mwl8k_save_beacon(struct ieee80211_hw *hw , struct sk_buff *skb ) 
{ struct mwl8k_priv *priv ;
 ...
 static struct mwl8k_vif *mwl8k_find_vif_bss(struct list_head *vif_list ,
                                                     u8 *bssid ) 
...
{ struct mwl8k_priv *priv ;
 ...
 static void mwl8k_tx_start(struct mwl8k_priv *priv ) 
{ 
...
{ struct mwl8k_priv *priv ;
 ...
 static bool mwl8k_ampdu_allowed(struct ieee80211_sta *sta , u8 tid ) 
{ struct mwl8k_sta *sta_info ;
 ...
 static void mwl8k_tx_count_packet(struct ieee80211_sta *sta , u8 tid ) 
{ struct mwl8k_sta *sta_info ;
  struct tx_traffic_info *tx_stats ;
  long tmp ;
...
{ struct mwl8k_priv *priv ;
  ...
  cmd->action = 1U;
  if ((int )priv->radio_short_preamble) {
    cmd->control = 3U;
  } else {
    cmd->control = 1U;
  }
...
{ struct mwl8k_cmd_finalize_join *cmd ;
  ...
  if (dtim != 0) {
    cmd->sleep_interval = (unsigned int )dtim;
  } else {
    cmd->sleep_interval = 1U;
  }
...
{ struct mwl8k_priv *priv ;
 ...
 static int mwl8k_cmd_set_mac_addr(struct ieee80211_hw *hw , struct ieee80211_vif *vif ,
                                           u8 *mac ) 
{ int tmp ;
 ...
 static int mwl8k_cmd_del_mac_addr(struct ieee80211_hw *hw , struct ieee80211_vif *vif ,
                                           u8 *mac ) 
{ int tmp ;
...
{ struct mwl8k_cmd_bss_start *cmd ;
  ...
  if (rc == 0) {
    if (enable != 0) {
      priv->running_bsses |= (u32 )(1 << mwl8k_vif->macid);
    } else {
      priv->running_bsses &= (u32 )(~ (1 << mwl8k_vif->macid));
    }
  }
...
{ struct mwl8k_priv *priv ;
  ...
  priv->macids_used |= (u32 )(1 << mwl8k_vif->macid);
...
{ 
  ...
  priv->macids_used &= (u32 )(~ (1 << vif->macid));
  list_del(& vif->list);
  return;
}
}
...
{ struct mwl8k_priv *priv ;
  ...
  _dev_info((struct device  const  *)(& (hw->wiphy)->dev), "%s v%d, %pm, %s firmware %u.%u.%u.%u\n",
            ...
            tmp, priv->fw_rev >> 24, (priv->fw_rev >> 16) & 255U, (priv->fw_rev >> 8) & 255U,
            priv->fw_rev & 255U);
  return (0);
  err_free_irq: 
  iowrite32(0U, priv->regs + 3124UL);
