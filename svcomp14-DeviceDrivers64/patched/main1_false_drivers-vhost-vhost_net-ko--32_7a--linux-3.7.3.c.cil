...
extern void __init_waitqueue_head(wait_queue_head_t * , char const   * , struct lock_class_key * ) ;
 static void init_waitqueue_func_entry(wait_queue_t *q , int (*func)(wait_queue_t * ,
                                                                             unsigned int  ,
                                                                             int  ,
                                                                             void * ) ) 
{ 
...
{ struct task_struct *tmp ;
  ...
  {
  tmp = get_current();
  if ((unsigned long )dev->mm == (unsigned long )tmp->mm) {
    tmp___0 = 0L;
  } else {
    tmp___0 = -1L;
  }
...
{ struct vhost_memory_region  const  *reg ;
  ...
  _iov->iov_len = (__kernel_size_t )tmp___3;
...
{ __u16 old ;
  ...
  if (tmp != 0) {
    tmp___0 = ldv__builtin_expect((int )vq->avail_idx == (int )vq->last_avail_idx,
                                  0L);
    if (tmp___0 != 0L) {
      return (1);
    }
...
extern struct socket *macvtap_get_socket(struct file * ) ;
 static bool sock_flag(struct sock  const  *sk , enum sock_flags flag ) 
{ int tmp ;
...
{ long tmp ;
  ...
  if (tmp != 0L) {
    tmp___0 = sock_flag((struct sock  const  *)sock->sk, 25);
    if ((int )tmp___0) {
      tmp___1 = 1;
    } else {
      tmp___1 = 0;
...
{ int seg ;
  ...
  _min1 = from->iov_len;
  _min2 = len;
  if (_min1 < (unsigned long )((unsigned long const   )_min2)) {
    size = _min1;
  } else {
    size = (unsigned long const   )_min2;
  }
  to->iov_base = from->iov_base;
  to->iov_len = size;
  len -= size;
...
{ struct vhost_virtqueue *vq ;
  ...
  if ((unsigned int )head == vq->num) {
    ...
    if (tmp___1 != 0L) {
      ...
      num_pends = (vq->upend_idx + 1024) - vq->done_idx;
    }
...
{ unsigned int out ;
  ...
  (heads + ((unsigned long )headcount + 0xffffffffffffffffUL))->len += (__u32 )datalen;
...
{ struct vhost_virtqueue *vq ;
  ...
  tmp___3 = get_rx_bufs(vq, vq->heads, (int )vhost_len, & in, vq_log, & log, tmp___19);
