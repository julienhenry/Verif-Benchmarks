...
{ struct cx25840_state *state ;
  ...
  if ((std & 16711680ULL) != 0ULL) {
    ...
    cx25840_write(client, 1183, tmp___2);
  }
  ...
  if ((std & 63744ULL) != 0ULL) {
    ...
    if (std == 8192ULL) {
      ...
      cx25840_write(client, 2056, tmp___3);
    } else
    if (std == 32768ULL) {
      if (hw_fix != 0) {
        tmp___4 = 63;
      ...
      cx25840_write(client, 2056, tmp___4);
    } else {
      if (hw_fix != 0) {
        tmp___5 = 31;
      } else {
      ...
      cx25840_write(client, 2056, tmp___5);
    }
    cx25840_write(client, 2059, 0);
  } else
  if ((std & 255ULL) != 0ULL) {
...
{ struct cx25840_state *state ;
  ...
  if ((unsigned int )is_component != 0U) {
    ...
    cx25840_and_or(client, 1025, 4294967289U, tmp___23);
  }
  ...
  if (tmp___15) {
    ...
    if (tmp___17) {
      ...
      cx25840_and_or(client, 258, 4294967293U, tmp___24);
    ...
    cx25840_and_or(client, 258, 4294967291U, tmp___25);
    if ((unsigned int )is_composite != 0U) {
...
{ struct cx25840_state *state ;
  ...
  Vlines = (int )(fmt->height + tmp___13);
...
{ struct cx25840_state *state ;
  ...
  if (((int )download_ctl & 16) != 0) {
    if (((int )mute_ctl & 2) != 0) {
      tmp___19 = (char *)"detecting";
    } else {
      tmp___19 = (char *)"running";
    }
    tmp___20 = tmp___19;
  } else {
    tmp___20 = (char *)"stopped";
  }
  printk("\016%s %d-%04x: Audio microcontroller:     %s\n", (client->driver)->driver.name,
         tmp___10, (int )client->addr, tmp___20);
...
{ struct cx25840_state *state ;
  ...
  if (cx25840_debug > 1) {
    ...
    printk("\017%s %d-%04x: AV Core IRQ status (entry): %s %s %s\n", (c->driver)->driver.name,
           tmp___1, (int )c->addr, tmp___11, tmp___10, tmp___9);
  }
  ...
  if (cx25840_debug > 1) {
    ...
    printk("\017%s %d-%04x: AV Core IRQ status (exit): %s %s %s\n", (c->driver)->driver.name,
           tmp___8, (int )c->addr, tmp___14, tmp___13, tmp___12);
  }
  return (ret);
}
}
...
{ struct cx25840_state *state ;
  ...
  case 243U: 
  tmp___12 = i2c_adapter_id(client->adapter);
  if (((int )device_id & 15) <= 2) {
    tmp___19 = ((int )device_id & 15) + 1;
  } else {
    tmp___19 = (int )device_id & 15;
  }
  printk("\016%s %d-%04x: cx25%3x-2%x found @ 0x%x (%s)\n", (client->driver)->driver.name,
         tmp___12, (int )client->addr, (int )device_id >> 4, tmp___19, (int )client->addr << 1,
         (char *)(& (client->adapter)->name));
  goto ldv_26770;
  case 236U: ;
  case 237U: ;
...
{ struct v4l2_ctrl *var_group1 ;
  ...
  if (ldv_s_cx25840_driver_i2c_driver == 0) {
    ...
    ldv_s_cx25840_driver_i2c_driver ++;
  }
  goto ldv_26852;
  case 22: ;
...
{ struct cx25840_state *state ;
 ...
 static int cx25836_set_audclk_freq(struct i2c_client *client , u32 freq ) 
{ int tmp ;
...
{ struct i2c_client *client ;
  ...
  cx25840_write(client, 1140, tmp___4);
  return (0);
}
}
int cx25840_decode_vbi_line(struct v4l2_subdev *sd , struct v4l2_decode_vbi_line *vbi ) 
...
extern unsigned int __kfifo_out_r(struct __kfifo * , void * , unsigned int  , size_t  ) ;
 static void init_ir_raw_event(struct ir_raw_event *ev ) 
{ 
...
static unsigned int ir_debug  ;
 static struct cx25840_ir_state *to_ir_state(struct v4l2_subdev *sd ) 
{ struct cx25840_state *state ;
  struct cx25840_state *tmp ;
...
{ 
 ...
 static u16 carrier_freq_to_clock_divider(unsigned int freq ) 
{ int __x ;
  unsigned int __d ;
  u16 tmp ;
 ...
 static unsigned int clock_divider_to_carrier_freq(unsigned int divider ) 
{ int __x ;
  unsigned int __d ;
 ...
 static unsigned int clock_divider_to_freq(unsigned int divider , unsigned int rollovers ) 
{ int __x ;
  unsigned int __d ;
...
{ 
 ...
 static u16 ns_to_lpf_count(unsigned int ns ) 
{ unsigned int __x ;
  int __d ;
  u16 tmp ;
 ...
 static unsigned int lpf_count_to_ns(unsigned int count ) 
{ unsigned int __x ;
  int __d ;
 ...
 static unsigned int lpf_count_to_us(unsigned int count ) 
{ unsigned int __x ;
  int __d ;
...
{ uint32_t __base ;
 ...
 static void control_tx_irq_watermark(struct i2c_client *c , enum tx_fifo_watermark level ) 
{ 
 ...
 static void control_rx_irq_watermark(struct i2c_client *c , enum rx_fifo_watermark level ) 
{ 
 ...
 static void control_tx_enable(struct i2c_client *c , bool enable ) 
{ unsigned int tmp ;
  ...
  cx25840_and_or4(c, 512, 4294966655U, tmp);
  return;
}
}
 static void control_rx_enable(struct i2c_client *c , bool enable ) 
{ unsigned int tmp ;
  ...
  cx25840_and_or4(c, 512, 4294966975U, tmp);
  return;
}
}
 static void control_tx_modulation_enable(struct i2c_client *c , bool enable ) 
{ unsigned int tmp ;
  ...
  cx25840_and_or4(c, 512, 4294967263U, tmp);
  return;
}
}
 static void control_rx_demodulation_enable(struct i2c_client *c , bool enable ) 
{ unsigned int tmp ;
  ...
  cx25840_and_or4(c, 512, 4294967279U, tmp);
  return;
}
}
 static void control_rx_s_edge_detection(struct i2c_client *c , u32 edge_types ) 
{ 
...
{ u32 v ;
 ...
 static void control_tx_polarity_invert(struct i2c_client *c , bool invert ) 
{ unsigned int tmp ;
  ...
  cx25840_and_or4(c, 512, 4294963199U, tmp);
  return;
}
}
...
{ u32 count ;
 ...
 static void irqenable_rx(struct v4l2_subdev *sd , u32 mask ) 
{ struct cx25840_state *state ;
  struct cx25840_state *tmp ;
  bool tmp___0 ;
  bool tmp___1 ;
 ...
 static void irqenable_tx(struct v4l2_subdev *sd , u32 mask ) 
{ struct cx25840_state *state ;
  struct cx25840_state *tmp ;
  bool tmp___0 ;
  bool tmp___1 ;
...
{ struct cx25840_state *state ;
  ...
  *handled = 0;
  ...
  if (ir_debug > 1U) {
    ...
    printk("\017%s: IR IRQ Status:  %s %s %s %s %s %s\n", (char *)(& sd->name), tmp___17,
           tmp___16, tmp___15, tmp___14, tmp___13, tmp___12);
  }
  if (ir_debug > 1U) {
    if (roe != 0) {
      tmp___18 = (char *)"roe";
...
{ struct cx25840_state *state ;
  ...
  printk("\016%s: \tPulse timers\' start/stop trigger:  %s\n", (char *)(& sd->name),
         s);
  if ((cntrl & 16384U) != 0U) {
    tmp___20 = (char *)"not loaded";
  } else {
    tmp___20 = (char *)"overflow marker";
  }
  printk("\016%s: \tFIFO data on pulse timer overflow: %s\n", (char *)(& sd->name),
         tmp___20);
  if ((cntrl & 1024U) != 0U) {
    tmp___21 = (char *)"not empty";
  } else {
    tmp___21 = (char *)"half full or greater";
  }
  printk("\016%s: \tFIFO interrupt watermark:          %s\n", (char *)(& sd->name),
         tmp___21);
  if ((cntrl & 8192U) != 0U) {
    tmp___22 = (char *)"loopback active";
  } else {
    tmp___22 = (char *)"normal receive";
  }
  ...
  printk("\016%s: \tMax measurable pulse width:        %u us, %llu ns\n", (char *)(& sd->name),
         tmp___11, tmp___10);
  if (filtr != 0U) {
    tmp___23 = (char *)"enabled";
  } else {
    tmp___23 = (char *)"disabled";
  }
  ...
  printk("\016%s: \tPulse width timer timed-out:       %s\n", (char *)(& sd->name),
         tmp___24);
  if ((int )irqen & 1) {
    tmp___25 = (char *)"enabled";
  } else {
    tmp___25 = (char *)"disabled";
  }
  ...
  printk("\016%s: \tFIFO overrun:                      %s\n", (char *)(& sd->name),
         tmp___26);
  if ((irqen & 2U) != 0U) {
    tmp___27 = (char *)"enabled";
  } else {
    tmp___27 = (char *)"disabled";
  }
  ...
  printk("\016%s: \tFIFO service requested:            %s\n", (char *)(& sd->name),
         tmp___29);
  if ((irqen & 16U) != 0U) {
    tmp___30 = (char *)"enabled";
  } else {
    tmp___30 = (char *)"disabled";
  }
  ...
  printk("\016%s: \tEnabled:                           %s\n", (char *)(& sd->name),
         tmp___31);
  if ((cntrl & 32U) != 0U) {
    tmp___32 = (char *)"enabled";
  } else {
    tmp___32 = (char *)"disabled";
  }
  printk("\016%s: \tModulation onto a carrier:         %s\n", (char *)(& sd->name),
         tmp___32);
  if ((cntrl & 128U) != 0U) {
    tmp___33 = (char *)"enabled";
  } else {
    tmp___33 = (char *)"disabled";
  }
  printk("\016%s: \tFIFO:                              %s\n", (char *)(& sd->name),
         tmp___33);
  if ((cntrl & 2048U) != 0U) {
    tmp___34 = (char *)"not empty";
  } else {
    tmp___34 = (char *)"half full or less";
  }
  printk("\016%s: \tFIFO interrupt watermark:          %s\n", (char *)(& sd->name),
         tmp___34);
  if ((cntrl & 4096U) != 0U) {
    tmp___35 = (char *)"space:burst mark:noburst";
  } else {
    tmp___35 = (char *)"space:noburst mark:burst";
  }
  ...
  printk("\016%s: \tFIFO service requested:            %s\n", (char *)(& sd->name),
         tmp___37);
  if ((irqen & 32U) != 0U) {
    tmp___38 = (char *)"enabled";
  } else {
    tmp___38 = (char *)"disabled";
  }
  printk("\016%s: \tFIFO service request interrupt:    %s\n", (char *)(& sd->name),
         tmp___38);
  return (0);
